...
int bmjpeg_ioctl_get_chn(int chn_fd, int *chn_id)
{
    int ret = 0;
    int is_jpeg = 1;
    *chn_id = ioctl(chn_fd, CVI_VC_VCODEC_GET_CHN, &is_jpeg);
    return ret;
}
...
{
    int ret = 0;
    ret = ioctl(chn_fd, CVI_VC_VDEC_GET_CHN_PARAM, pstParam);
    return ret;
}
...
{
    int ret = 0;
    ret = ioctl(chn_fd, CVI_VC_VDEC_SET_CHN_PARAM, pstParam);
    return ret;
}
...
{
    int ret = 0;
    ret = ioctl(chn_fd, CVI_VC_VENC_DESTROY_CHN, NULL);
    return ret;
}
...
{
    int ret = 0;
    ret = ioctl(chn_fd, CVI_VC_VENC_STOP_RECV_FRAME, NULL);
    return ret;
}
...
{
    int ret = 0;
    ret = ioctl(chn_fd, CVI_VC_VENC_QUERY_STATUS, pstStatus);
    return ret;
}
...
{
    int ret = 0;
    ret = ioctl(chn_fd, CVI_VC_VENC_RESET_CHN, NULL);
    return ret;
}