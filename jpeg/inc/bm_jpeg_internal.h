...
#define BM_JPU_ERROR_FULL(FILE_, LINE_, FUNCTION_, ...)   do { if (bm_jpu_cur_log_level_threshold >= BM_JPU_LOG_LEVEL_ERROR)   { bm_jpu_logging_fn(BM_JPU_LOG_LEVEL_ERROR,   FILE_, LINE_, FUNCTION_, __VA_ARGS__); } } while(0)
#define BM_JPU_WARNING_FULL(FILE_, LINE_, FUNCTION_, ...) do { if (bm_jpu_cur_log_level_threshold >= BM_JPU_LOG_LEVEL_WARNING) { bm_jpu_logging_fn(BM_JPU_LOG_LEVEL_WARNING, FILE_, LINE_, FUNCTION_, __VA_ARGS__); } } while(0)
#define BM_JPU_INFO_FULL(FILE_, LINE_, FUNCTION_, ...)    do { if (bm_jpu_cur_log_level_threshold >= BM_JPU_LOG_LEVEL_INFO)    { bm_jpu_logging_fn(BM_JPU_LOG_LEVEL_INFO,    FILE_, LINE_, FUNCTION_, __VA_ARGS__); } } while(0)
#define BM_JPU_DEBUG_FULL(FILE_, LINE_, FUNCTION_, ...)   do { if (bm_jpu_cur_log_level_threshold >= BM_JPU_LOG_LEVEL_DEBUG)   { bm_jpu_logging_fn(BM_JPU_LOG_LEVEL_DEBUG,   FILE_, LINE_, FUNCTION_, __VA_ARGS__); } } while(0)
#define BM_JPU_LOG_FULL(FILE_, LINE_, FUNCTION_, ...)     do { if (bm_jpu_cur_log_level_threshold >= BM_JPU_LOG_LEVEL_LOG)     { bm_jpu_logging_fn(BM_JPU_LOG_LEVEL_LOG,     FILE_, LINE_, FUNCTION_, __VA_ARGS__); } } while(0)
#define BM_JPU_TRACE_FULL(FILE_, LINE_, FUNCTION_, ...)   do { if (bm_jpu_cur_log_level_threshold >= BM_JPU_LOG_LEVEL_TRACE)   { bm_jpu_logging_fn(BM_JPU_LOG_LEVEL_TRACE,   FILE_, LINE_, FUNCTION_, __VA_ARGS__); } } while(0)
