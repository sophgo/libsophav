...
RetCode Wave5VpuDecRegisterFramebuffer(CodecInst* inst, FrameBuffer* fbArr, TiledMapType mapType, Uint32 count)
{
    ...
    Uint32       scalerFlag = (mapType == LINEAR_FRAME_MAP) ? pDecInfo->scalerInfo.enScaler : 0;
    Uint32       pixelOrder=1;
    ...
    else
    {
        ...
        if (pDecInfo->scalerInfo.enScaler == TRUE) {
            picSize = (pDecInfo->scalerInfo.scaleWidth << 16) | (pDecInfo->scalerInfo.scaleHeight);
        }
    }
...
RetCode Wave5VpuDecUpdateFramebuffer(CodecInst* inst, FrameBuffer* fbcFb, FrameBuffer* linearFb, Int32 mvIndex, Int32 picWidth, Int32 picHeight)
{
    ...
    if (pDecInfo->scalerInfo.enScaler == TRUE) {
        regVal = (pDecInfo->scalerInfo.scaleWidth << 16) | (pDecInfo->scalerInfo.scaleHeight);
    }
...
RetCode Wave5VpuDecGetResult(CodecInst* instance, DecOutputInfo* result)
{
    ...
    if (pDecInfo->scalerInfo.enScaler && result->sequenceChanged && (instance->codecMode != W_VP9_DEC)) {
        pDecInfo->scalerInfo.scaleWidth  = pDecInfo->newSeqInfo.picWidth;
        pDecInfo->scalerInfo.scaleHeight = pDecInfo->newSeqInfo.picHeight;
    }
    return RETCODE_SUCCESS;
}
    ...
    case DEC_GET_QUEUE_STATUS:
        {
            ...
            queueInfo->instanceQueueFull = (decInfo->instanceQueueCount == COMMAND_QUEUE_DEPTH) ? TRUE : FALSE;
    ...
    case SET_SEC_AXI:
        {
            ...
            if (secAxiUse.u.wave.useBitEnable) {
                ...
                if (instance->codecMode == W_AVC_DEC) {
                    if ((decInfo->productCode != WAVE511_CODE) && (decInfo->productCode != WAVE521C_DUAL_CODE)) {
                        return RETCODE_INVALID_PARAM;
                    }
                }
            }
            ...
            decInfo->secAxiInfo.u.wave.useSclEnable   = secAxiUse.u.wave.useSclEnable;
            break;
        }
    ...
    case DEC_SET_SCALER_INFO:
        {
            ...
            if (!decInfo->initialInfoObtained)
            ...
            if (scalerInfo.enScaler == TRUE) {
                ...
                Uint32   minScaleWidth  = (pAttr->support2AlignScaler == TRUE) ? VPU_ALIGN2((decInfo->initialInfo.picWidth/8))  : VPU_ALIGN8((decInfo->initialInfo.picWidth/8));
                Uint32   minScaleHeight = (pAttr->support2AlignScaler == TRUE) ? VPU_ALIGN2((decInfo->initialInfo.picHeight/8)) : VPU_ALIGN8((decInfo->initialInfo.picHeight/8));
                ...
                if ((scalerInfo.scaleWidth % scalerStep) || scalerInfo.scaleWidth > (Uint32)(VPU_ALIGN8(decInfo->initialInfo.picWidth))) {
                    VLOG(ERR, "scaleWidth: %d\n", scalerInfo.scaleWidth);
                    return RETCODE_INVALID_PARAM;
                }
                if ((scalerInfo.scaleHeight % scalerStep) || scalerInfo.scaleHeight > (Uint32)(VPU_ALIGN8(decInfo->initialInfo.picHeight))) {
                    VLOG(ERR, "scaleHeight: %d\n", scalerInfo.scaleHeight);
                    return RETCODE_INVALID_PARAM;
                }

                decInfo->scalerInfo = scalerInfo;
            }
            break;
        }
    default:
    ...
    case ENC_GET_QUEUE_STATUS:
        {
            ...
            queueInfo->instanceQueueFull  = (pEncInfo->instanceQueueCount == COMMAND_QUEUE_DEPTH) ? TRUE : FALSE;
