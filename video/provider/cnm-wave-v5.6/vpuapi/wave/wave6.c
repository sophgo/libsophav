...
RetCode Wave6VpuBuildUpDecParam(CodecInst* instance, DecOpenParam* param)
{
    ...
    pDecInfo->scalerInfo.scaleWidth   = 0;
    pDecInfo->scalerInfo.scaleHeight  = 0;
...
RetCode Wave6VpuDecRegisterFramebuffer(CodecInst* inst, FrameBuffer* fbArr, TiledMapType mapType, Uint32 count)
{
    ...
    if (mapType >= COMPRESSED_FRAME_MAP) {
        ...
        picSize = (initPicWidth<<16)|(initPicHeight);
        if (info->scalerInfo.enScaler == TRUE) {
            picSize = (info->scalerInfo.scaleWidth << 16) | (info->scalerInfo.scaleHeight);
        }
        ...
        VpuWriteReg(coreIdx, W6_CMD_DEC_SET_FB_SCL_PARAM, (colorFormat                <<3) |
                                                          (info->scalerInfo.scaleMode <<1) |
                                                          (info->scalerInfo.enScaler  <<0));
    }
...
RetCode Wave6VpuDecUpdateFramebuffer(CodecInst* inst, FrameBuffer* fbcFb, FrameBuffer* linearFb, Int32 mvIndex, Int32 width, Int32 height)
{
    ...
    if (fbcFb->myIndex >= 0) {
        ...
        fbcCTbl = 0;
        mvCol   = 0;
        if (info->scalerInfo.enScaler) {
            width  = info->scalerInfo.scaleWidth;
            height = info->scalerInfo.scaleHeight;
        }
    }
    else if (mvIndex >= 0) {
        stride  = 0;
...
RetCode Wave6VpuDecGetResult(CodecInst* instance, DecOutputInfo* result)
{
    ...
    if (info->scalerInfo.enScaler && result->sequenceChanged && (instance->codecMode != W_VP9_DEC)) {
        info->scalerInfo.scaleWidth  = info->newSeqInfo.picWidth;
        info->scalerInfo.scaleHeight = info->newSeqInfo.picHeight;
    }
    ...
    case DEC_SET_WTL_FRAME_FORMAT:
        {
            ...
            if ((frameFormat != FORMAT_420)               &&
                ...
                (frameFormat != FORMAT_444)               &&
                (frameFormat != FORMAT_444_P10_16BIT_MSB) &&
                (frameFormat != FORMAT_444_P10_16BIT_LSB) &&
                (frameFormat != FORMAT_444_P10_32BIT_MSB) &&
                (frameFormat != FORMAT_444_P10_32BIT_LSB) &&
    ...
    case DEC_SET_SCALER_INFO:
        {
            ...
            if (scalerInfo.enScaler == TRUE) {
                ...
                Uint32   minScaleWidth  = (pAttr->support2AlignScaler == TRUE) ? VPU_ALIGN2((info->initialInfo.picWidth/8))  : VPU_ALIGN8((info->initialInfo.picWidth/8));
                Uint32   minScaleHeight = (pAttr->support2AlignScaler == TRUE) ? VPU_ALIGN2((info->initialInfo.picHeight/8)) : VPU_ALIGN8((info->initialInfo.picHeight/8));

                if (minScaleWidth == 0)  minScaleWidth  = scalerStep;
                if (minScaleHeight == 0) minScaleHeight = scalerStep;

                if (scalerInfo.scaleWidth == 0 || scalerInfo.scaleHeight == 0) {
                    VLOG(ERR, "scaleWidth: %d | scaleHeight: %d\n", scalerInfo.scaleWidth, scalerInfo.scaleHeight);
                    return RETCODE_INVALID_PARAM;
                }
                if (scalerInfo.scaleWidth < minScaleWidth || scalerInfo.scaleHeight < minScaleHeight) {
                    VLOG(ERR, "scaleWidth: %d  < minScaleWidth: %d | scaleHeight: %d < minScaleHeight: %d\n", scalerInfo.scaleWidth, minScaleWidth, scalerInfo.scaleHeight, minScaleHeight);
                    return RETCODE_INVALID_PARAM;
                }
                if ((scalerInfo.scaleWidth % scalerStep) || scalerInfo.scaleWidth > (Uint32)(VPU_ALIGN8(info->initialInfo.picWidth))) {
                    VLOG(ERR, "scaleWidth: %d\n", scalerInfo.scaleWidth);
                    return RETCODE_INVALID_PARAM;
                }
                if ((scalerInfo.scaleHeight % scalerStep) || scalerInfo.scaleHeight > (Uint32)(VPU_ALIGN8(info->initialInfo.picHeight))) {
                    VLOG(ERR, "scaleHeight: %d\n", scalerInfo.scaleHeight);
                    return RETCODE_INVALID_PARAM;
                }

                info->scalerInfo = scalerInfo;
            }
            break;
        }
    default:
...
RetCode Wave6VpuEncRegisterFramebuffer(CodecInst* inst, FrameBuffer* fbArr)
{
    ...
    osal_memset(&reg, 0x00, sizeof(struct enc_cmd_set_fb_reg));
...
RetCode Wave6VpuEncode(CodecInst* inst, EncParam* opt)
{
    ...
    osal_memset(&reg, 0x00, sizeof(struct enc_cmd_enc_pic_reg));
...
RetCode Wave6VpuEncCheckOpenParam(EncOpenParam open_param)
{
    ...
    if (open_param.bitstreamFormat == STD_HEVC) {
    ...
    else if (open_param.bitstreamFormat == STD_AV1) {
        if (param.idrPeriod != 0) {
            VLOG(ERR, "idrPeriod: %d\n", param.idrPeriod);
            return RETCODE_INVALID_PARAM;
        }
        ...
        if (param.constrainedIntraPred != FALSE) {
            VLOG(ERR, "constrainedIntraPred: %d\n", param.constrainedIntraPred);
            return RETCODE_INVALID_PARAM;
        }
        if (param.intraTransSkip != TRUE && param.intraTransSkip != FALSE) {
        ...
        if (param.lfCrossSliceBoundaryFlag != FALSE) {
            VLOG(ERR, "lfCrossSliceBoundaryFlag: %d\n", param.lfCrossSliceBoundaryFlag);
            return RETCODE_INVALID_PARAM;
        }
        if (param.betaOffsetDiv2 != 0) {
            VLOG(ERR, "betaOffsetDiv2: %d\n", param.betaOffsetDiv2);
            return RETCODE_INVALID_PARAM;
        }
        if (param.tcOffsetDiv2 != 0) {
            VLOG(ERR, "tcOffsetDiv2: %d\n", param.tcOffsetDiv2);
            return RETCODE_INVALID_PARAM;
        }
        if (param.lfSharpness > 7) {
            VLOG(ERR, "lfSharpness: %d\n", param.lfSharpness);
            return RETCODE_INVALID_PARAM;
        }
        if (param.enSAO != FALSE) {
        ...
        if (param.enCdef != TRUE && param.enCdef != FALSE) {
            VLOG(ERR, "enCdef: %d\n", param.enCdef);
            return RETCODE_INVALID_PARAM;
        }
        if (param.enWiener != TRUE && param.enWiener != FALSE) {
            VLOG(ERR, "enWiener: %d\n", param.enWiener);
            return RETCODE_INVALID_PARAM;
        }
        if (param.yDcQpDelta < -64 || param.yDcQpDelta > 63) {
            VLOG(ERR, "yDcQpDelta: %d\n", param.yDcQpDelta);
            return RETCODE_INVALID_PARAM;
        }
        if (param.uDcQpDelta < -64 || param.uDcQpDelta > 63) {
            VLOG(ERR, "uDcQpDelta: %d\n", param.uDcQpDelta);
            return RETCODE_INVALID_PARAM;
        }
        if (param.vDcQpDelta < -64 || param.vDcQpDelta > 63) {
            VLOG(ERR, "vDcQpDelta: %d\n", param.vDcQpDelta);
            return RETCODE_INVALID_PARAM;
        }
        if (param.uAcQpDelta < -64 || param.uAcQpDelta > 63) {
            VLOG(ERR, "uAcQpDelta: %d\n", param.uAcQpDelta);
            return RETCODE_INVALID_PARAM;
        }
        if (param.vAcQpDelta < -64 || param.vAcQpDelta > 63) {
            VLOG(ERR, "vAcQpDelta: %d\n", param.vAcQpDelta);
            return RETCODE_INVALID_PARAM;
        }
        if (param.cbQpOffset != 0) {
            VLOG(ERR, "cbQpOffset: %d\n", param.cbQpOffset);
            return RETCODE_INVALID_PARAM;
        }
        if (param.crQpOffset != 0) {
            VLOG(ERR, "crQpOffset: %d\n", param.crQpOffset);
            return RETCODE_INVALID_PARAM;
        }
        if (param.enStillPicture != FALSE) {
