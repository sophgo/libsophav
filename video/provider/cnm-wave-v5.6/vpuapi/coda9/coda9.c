...
RetCode Coda9VpuEncSetup(CodecInst* pCodecInst)
{
    ...
    //======================================================================================
    // using RC library,
    //   1. calculate non-roi QP considering target bitrate when rc mode == 4 && roi enabled.
    //   2. calculate picQp (rc mode==4). if rc mode is 4(picture level RC) && roi disabled
    //======================================================================================
    if (pEncInfo->openParam.rcEnable == 4) {
        int pic_width, pic_height;
        int bps = pEncInfo->openParam.bitRate*1000;
        ...
        pEncInfo->prevWrPtr = pEncInfo->streamBufStartAddr; // to calculate real encoded bit when ringbuffer enabled.
...
#endif
            frm_rc_seq_init(&pEncInfo->frm_rc, bps, buf_size_ms, frame_rate, pic_width, pic_height, gop_size, pEncInfo->openParam.rcEnable,
            pEncInfo->openParam.set_dqp_pic_num, pEncInfo->openParam.dqp, pEncInfo->openParam.RcInitialQp
            , is_first_pic,
            gamma,
    ...
    if (rcEnable) { // rate control enabled
        if (productId == PRODUCT_ID_980) {
            if (pEncInfo->openParam.bitstreamFormat == STD_AVC) {
                int MinDeltaQp, MaxDeltaQp, QpMinI, QpMaxI, QpMinP, QpMaxP;
                ...
                data = (pEncInfo->openParam.HvsQpScaleDiv2 << 25) |(pEncInfo->openParam.EnHvsQp <<24) | (pEncInfo->openParam.EnRowLevelRc <<23) | (pEncInfo->openParam.bitRate<<4)|(rcEnable);
                ...
                VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_SEQ_DELTA_QP, data);
    ...
    if (pCodecInst->codecMode == AVC_ENC) {
        ...
        if (pCodecInst->codecModeAux == AVC_AUX_MVC) {
            data |= (pEncInfo->openParam.EncStdParam.avcParam.interviewEn << 4);
            data |= (pEncInfo->openParam.EncStdParam.avcParam.parasetRefreshEn << 8);
            data |= (pEncInfo->openParam.EncStdParam.avcParam.prefixNalEn << 9);
        }
...
RetCode Coda9VpuEncParaChange(CodecInst* pCodecInst, ParamChange* param)
{
    ...
    if (param->paraEnable & C9_ENC_CHANGE_PARAM_GOP_NUM) {
        int encodedFrames = pEncInfo->openParam.EncStdParam.avcParam.fieldFlag ? (2*pEncInfo->encoded_frames_in_gop) : (pEncInfo->encoded_frames_in_gop);
        if (encodedFrames - param->NewGopNum >= 0)
    ...
    if (param->paraEnable & C9_ENC_CHANGE_PARAM_MIN_MAX_QP) {
        VpuWriteReg(coreIdx, CMD_ENC_PARAM_CHANGE_MIN_MAX_QP, (param->minMaxQpParam.maxQpPEnable<<30) |
                                                              (param->minMaxQpParam.minQpP<<24)       |
                                                              (param->minMaxQpParam.maxQpPEnable<<22) |
                                                              (param->minMaxQpParam.maxQpP<<16)       |
                                                              (param->minMaxQpParam.maxQpIEnable<<14) |
                                                              (param->minMaxQpParam.minQpI<<8)        |
                                                              (param->minMaxQpParam.maxQpIEnable<<6)  |
                                                              (param->minMaxQpParam.maxQpI<<0));

        pEncInfo->openParam.userQpMaxI = param->minMaxQpParam.maxQpI;
        pEncInfo->openParam.userQpMinI = param->minMaxQpParam.minQpI;
        pEncInfo->openParam.userQpMaxP = param->minMaxQpParam.maxQpP;
        pEncInfo->openParam.userQpMinP = param->minMaxQpParam.minQpP;
    }
    if (param->paraEnable & C9_ENC_CHANGE_PARAM_PIC_PARAM) {
        int gamma = (param->changePicParam.GammaEn == TRUE) ? param->changePicParam.Gamma : 0;

        VpuWriteReg(coreIdx, CMD_ENC_PARAM_MIN_MAX_DELTA_QP, (param->changePicParam.MindeltaQp<<8) |
                                                             (param->changePicParam.MaxdeltaQp));
        VpuWriteReg(coreIdx, CMD_ENC_PARAM_PIC_RC,           (param->changePicParam.RcHvsMaxDeltaQp<<6) |
                                                             (param->changePicParam.HvsQpScaleDiv2<<2)  |
                                                             (param->changePicParam.EnHvsQp<<1)         |
                                                             (param->changePicParam.EnRowLevelRc));
        VpuWriteReg(coreIdx, CMD_ENC_PARAM_PIC_RC_2,         (param->changePicParam.RcInitDelay<<16)         |
                                                             (param->changePicParam.RcGopIQpOffsetEn<<5)     |
                                                             ((param->changePicParam.RcGopIQpOffset&0xF)<<1) |
                                                             (param->changePicParam.GammaEn));
        VpuWriteReg(coreIdx, CMD_ENC_PARAM_PIC_GAMMA,        gamma);

        pEncInfo->openParam.userMaxDeltaQp   = param->changePicParam.MaxdeltaQp;
        pEncInfo->openParam.userMinDeltaQp   = param->changePicParam.MindeltaQp;
        pEncInfo->openParam.HvsQpScaleDiv2   = param->changePicParam.HvsQpScaleDiv2;
        pEncInfo->openParam.EnHvsQp          = param->changePicParam.EnHvsQp;
        pEncInfo->openParam.EnRowLevelRc     = param->changePicParam.EnRowLevelRc;
        pEncInfo->openParam.RcHvsMaxDeltaQp  = param->changePicParam.RcHvsMaxDeltaQp;
        pEncInfo->openParam.rcInitDelay      = param->changePicParam.RcInitDelay;
        pEncInfo->openParam.userGamma        = param->changePicParam.Gamma;
        pEncInfo->openParam.set_dqp_pic_num  = param->changePicParam.SetDqpNum;
        pEncInfo->openParam.rcGopIQpOffsetEn = param->changePicParam.RcGopIQpOffsetEn;
        pEncInfo->openParam.rcGopIQpOffset   = param->changePicParam.RcGopIQpOffset;
        pEncInfo->openParam.rcWeightFactor   = param->changePicParam.rcWeightFactor;
        osal_memcpy(pEncInfo->openParam.dqp, param->changePicParam.dqp, sizeof(int)*8);
    }
...
RetCode Coda9VpuEncode(CodecInst* pCodecInst, EncParam* param)
{
    ...
    if(pCodecInst->productId == PRODUCT_ID_980 && pCodecInst->codecMode == AVC_ENC && pEncInfo->openParam.rcEnable == 4) {
        ...
        // outQp will be returned a QP for non-roi region when rc mode==4 && roi enabled.
        // outQp will be returned a QP for one picture when rc mode==4 && roi disabled.
        int min_qp;
        int max_qp;
        int pic_idx;
        int gop_num;
        ...
        mvc_second_view = pEncInfo->openParam.EncStdParam.avcParam.mvcExtension && (!pEncInfo->openParam.EncStdParam.avcParam.interviewEn);
        ...
        {
            if (encoded_frames >= gop_num)   // I frame
            {
                encoded_frames        = 0;
                pEncInfo->encoded_frames_in_gop = 0;
            }
        ...
        min_qp = slice_type == 0 ? pEncInfo->openParam.userQpMinI : pEncInfo->openParam.userQpMinP;
        max_qp = slice_type == 0 ? pEncInfo->openParam.userQpMaxI : pEncInfo->openParam.userQpMaxP;

        frm_rc_pic_init(&pEncInfo->frm_rc, !slice_type,
            min_qp, max_qp,
#ifdef CLIP_PIC_DELTA_QP
            &row_max_dqp_minus, &row_max_dqp_plus,
#endif
            &outQp, &target_pic_bit, &hrd_buf_level, &hrd_buf_size);
    }
    if (pCodecInst->productId == PRODUCT_ID_980) {
    ...
    if (pCodecInst->productId == PRODUCT_ID_980 && pCodecInst->codecMode == AVC_ENC)
    {
        ...
        {
            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_ROI_INFO, param->coda9RoiPicAvgQp<<1 | param->coda9RoiEnable); // currently, only mode 0 can be supported
            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_ROI_QP_MAP_ADDR, param->roiQpMapAddr);
            Coda9BitIssueCommand(pCodecInst->coreIdx, pCodecInst, ENC_ROI_INIT);
            if (vdi_wait_vpu_busy(pCodecInst->coreIdx, __VPU_BUSY_TIMEOUT, BIT_BUSY_FLAG) == -1) {
                if (pCodecInst->loggingEnable)
                    vdi_log(pCodecInst->coreIdx, pCodecInst->instIndex, ENC_ROI_INIT, 0);
                return RETCODE_VPU_RESPONSE_TIMEOUT;
            }
            if (pCodecInst->loggingEnable)
                vdi_log(pCodecInst->coreIdx, pCodecInst->instIndex, ENC_ROI_INIT, 0);
...

        {
            ...
            if (ActvePPS->entropyCodingMode==2)
            {
                ...
                if (pEncInfo->openParam.EncStdParam.avcParam.mvcExtension)
                {
                    if (pEncInfo->openParam.EncStdParam.avcParam.interviewEn)
                    {
                        ...
                        {
                            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PARAM_CHANGE_ENABLE, (C9_ENC_CHANGE_PARAM_PPS_ID | C9_ENC_CHANGE_PARAM_CABAC_MODE));
                        ...
                        else //(slice_type==0 && pic_idx%2 !=0)
                        {
                            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PARAM_CHANGE_ENABLE, (C9_ENC_CHANGE_PARAM_PPS_ID | C9_ENC_CHANGE_PARAM_CABAC_MODE));
                            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PARAM_CHANGE_CABAC_MODE, 1);
                            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PARAM_CHANGE_PPS_ID, 3);
                        }
                    }
                    else
                    {
                        ...
                        else if(slice_type == 0 && (pic_idx%2) !=0)
                        {
                            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PARAM_CHANGE_ENABLE, (C9_ENC_CHANGE_PARAM_PPS_ID | C9_ENC_CHANGE_PARAM_CABAC_MODE));
                            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PARAM_CHANGE_CABAC_MODE, 0);
                            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PARAM_CHANGE_PPS_ID, 2);
                        }
                ...
                else
                {
                    ...
                    {
                        if (slice_type != 0) // (encoded_frames_in_gop > 0)
                        {
                            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PARAM_CHANGE_CABAC_MODE, 1);
                            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PARAM_CHANGE_PPS_ID, 1);
                        }
            ...
            else
            {
                ...
                if (pEncInfo->openParam.EncStdParam.avcParam.mvcExtension)
                    gop_num *= 2;
                /*
                Currently slice_type is not used,
                But encoded_frame_in_gop should be managed by this block.
                */
                if (gop_num == 0)   // Only first I
                {
                    if (pic_idx == 0 || (pic_idx == 1 && mvc_second_view == 1))
    ...
    if (param->skipPicture) {
        ...
        if (pEncInfo->openParam.cbcrOrder == CBCR_ORDER_NORMAL) {
            ...
            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PIC_SRC_BOTTOM_Y, pSrcFrame->bufYBot);
            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PIC_SRC_BOTTOM_CB, pSrcFrame->bufCrBot);
            VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PIC_SRC_BOTTOM_CR, pSrcFrame->bufCbBot);
        }
...
RetCode Coda9VpuEncGetResult(CodecInst* pCodecInst, EncOutputInfo* info)
{
    ...
    if (pCodecInst->productId == PRODUCT_ID_980 && pCodecInst->codecMode == AVC_ENC && pEncInfo->openParam.rcEnable == 4) {
        ...
        temp = VpuReadReg(pCodecInst->coreIdx, RET_ENC_PIC_RC);
        real_pic_bit = VpuReadReg(pCodecInst->coreIdx, RET_ENC_PIC_USED_BIT);

        frm_rc_pic_end(&pEncInfo->frm_rc, real_pic_bit, (temp & 0x7E) >>1, temp & 0x1);
...
RetCode Coda9VpuEncGetHeader(CodecInst* pCodecInst, EncHeaderParam* encHeaderParam)
{
    ...
    if ((encHeaderParam->headerType == SPS_RBSP || encHeaderParam->headerType == SPS_RBSP_MVC) &&
        ...
        VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_HEADER_FIELD_FLAG, (encOP->EncStdParam.avcParam.fieldRefMode << 1) | encOP->EncStdParam.avcParam.fieldFlag);
...
//                    (pSubFrameSyncState->ipuCurFrameIndex<<2) | ((pSubFrameSyncState->ipuNewFrame&0x01)<<1) | (pSubFrameSyncState->ipuEndOfRow&0x01));
        ...
        break;
    case ENC_SET_RC_LIBRARY:
        {
            EncInfo* pEncInfo  = &pCodecInst->CodecInfo->encInfo;
            BOOL     fieldDone = *(BOOL *)param;
            int is_first_pic = (pEncInfo->frameIdx == 0 && fieldDone == FALSE);
            int frame_rate   = (pEncInfo->openParam.frameRateInfo&0xffff) / ((pEncInfo->openParam.frameRateInfo>>16) + 1);
            int gop_size     = pEncInfo->openParam.gopSize;
            int gamma        = pEncInfo->openParam.userGamma;
            int pic_width    = 0;
            ...
            if(gamma == -1) gamma = 1;
#ifdef CLIP_PIC_DELTA_QP
            if (max_delta_qp_minus == -1) max_delta_qp_minus = 51;
            if (max_delta_qp_plus  == -1) max_delta_qp_plus  = 51;
#endif /* CLIP_PIC_DELTA_QP */
            frm_rc_seq_init(&pEncInfo->frm_rc,
