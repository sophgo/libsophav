...
RetCode VPU_DecGetOutputInfo(DecHandle handle, DecOutputInfo* info)
{
    ...
    if (info->indexFrameDisplay >= 0 && info->indexFrameDisplay < MAX_GDI_IDX)
    {
        ...
        if (pDecInfo->scalerInfo.enScaler) {
            if ((pDecInfo->scalerInfo.scaleWidth != 0) && (pDecInfo->scalerInfo.scaleHeight != 0)) {
                info->dispPicWidth     = pDecInfo->scalerInfo.scaleWidth;
                info->dispPicHeight    = pDecInfo->scalerInfo.scaleHeight;
                info->rcDisplay.right  = pDecInfo->scalerInfo.scaleWidth;
                info->rcDisplay.bottom = pDecInfo->scalerInfo.scaleHeight;
            }
        }
    }
    else
...
RetCode VPU_DecGiveCommand(DecHandle handle, CodecCommand cmd, void* param)
{
    ...
    case DEC_GET_SCALER_INFO:
        ret = ProductVpuDecGiveCommand(pCodecInst, DEC_GET_SCALER_INFO, param);
        return ret;
    case DEC_SET_SCALER_INFO:
        ret = ProductVpuDecGiveCommand(pCodecInst, DEC_SET_SCALER_INFO, param);
        return ret;
    ...
    case ENC_SET_PARAM:
        {
            ...
            if (ret != RETCODE_SUCCESS)
                return ret;
            if (pEncInfo->openParam.EncStdParam.avcParam.mvcExtension == TRUE) {
                ret = EncParaSet(handle, SPS_RBSP_MVC);
                if (ret != RETCODE_SUCCESS)
                    return ret;

                ret = EncParaSet(handle, PPS_RBSP_MVC);
                if (ret != RETCODE_SUCCESS)
                    return ret;
            }
            return ret;
    ...
    case ENC_SET_BITRATE:
        {
            ...
            if ((pCodecInst->codecMode != MP4_ENC) && (pCodecInst->codecMode != AVC_ENC))
                return RETCODE_INVALID_COMMAND;
            if ((pCodecInst->codecMode == AVC_ENC) && (paramChange.NewBitrate < 0 || paramChange.NewBitrate > 524288))
                return RETCODE_INVALID_PARAM;
    ...
    case ENC_SET_MIN_MAX_QP:
        {
            ...
            paramChange.paraEnable    = C9_ENC_CHANGE_PARAM_MIN_MAX_QP;
            paramChange.minMaxQpParam = *(MinMaxQpChangeParam*)param;
    ...
    case ENC_SET_PIC_CHANGE_PARAM:
        {
            ...
            paramChange.paraEnable     = C9_ENC_CHANGE_PARAM_PIC_PARAM;
            paramChange.changePicParam = *(ChangePicParam*)param;
    ...
    case ENC_SET_RC_LIBRARY:
        {
            ret = EnterLock(pCodecInst->coreIdx);
            if (ret == RETCODE_SUCCESS) {
                ret = ProductVpuEncGiveCommand(pCodecInst, ENC_SET_RC_LIBRARY, param);
                LeaveLock(pCodecInst->coreIdx);
            }
            return ret;
        }
