...
RetCode EncParaSet(EncHandle handle, int paraSetType)
{
    ...
    if( (paraSetType == SPS_RBSP_MVC || paraSetType == SPS_RBSP) && pEncInfo->openParam.bitstreamFormat == STD_AVC) {
        ...
        VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_HEADER_FIELD_FLAG, encOP->EncStdParam.avcParam.fieldFlag|encOP->EncStdParam.avcParam.fieldFlag);
		...
		if (encOP->sliceMode.sliceMode==1 && encOP->sliceMode.sliceSizeMode==1)
			SliceNum = encOP->sliceMode.sliceSize;

		if (!encOP->EncStdParam.avcParam.level) {
            if (encOP->EncStdParam.avcParam.fieldFlag)
                encOP->EncStdParam.avcParam.level = LevelCalculation(picWidth/16, (picHeight+31)/32, encOP->frameRateInfo, 1, encOP->bitRate, SliceNum);
            else
                encOP->EncStdParam.avcParam.level = LevelCalculation(picWidth/16, picHeight/16, encOP->frameRateInfo, 0, encOP->bitRate, SliceNum);
    ...
    if( (paraSetType == PPS_RBSP ||  paraSetType == PPS_RBSP_MVC) && pEncInfo->openParam.bitstreamFormat == STD_AVC)
    {
        ...
        if (ActivePPS->entropyCodingMode != 2)
        {
            ...
            if (paraSetType == PPS_RBSP)
                ...
                spsID = 1;
                encHeaderCode |= ((ActivePPS->ppsId+1)<<16); /* sps_id : 1 */
                encHeaderCode |= (spsID<<24) | ((ActivePPS->ppsId+1)<<16); /* sps_id : 1 */
            }
...
{
    if (vdi_lock(coreIdx) != 0 || SetClockGate(coreIdx, 1) != 0)
        return RETCODE_FAILURE;
    return RETCODE_SUCCESS;
}
