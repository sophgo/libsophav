...
static void PrintUserData(DecListenerContext* ctx, CNMComListenerDecDone*  lsnpDecDone)
{
    ...
    VpuReadMem(VPU_HANDLE_CORE_INDEX(lsnpDecDone->handle), vbUser->phys_addr, pBase, vbUser->size, VPU_USER_DATA_ENDIAN);
...
void HandleDecCompleteSeqEvent(Component com, CNMComListenerDecCompleteSeq* param, DecListenerContext* ctx)
{
    ...
    Int32  chroma_format_idx = param->initialInfo->chromaFormatIDC;

    if (ctx->enableScaler == TRUE) {
        width  = VPU_CEIL(ctx->scale_width, 2);
        height = VPU_CEIL(ctx->scale_height, 2);
    }

    if (ctx->compareType == YUV_COMPARE) {
        Int32 comp_type = YUV_COMPARE;
        if ((ctx->comparator = Comparator_Create(comp_type, param->refYuvPath, width, height, param->wtlFormat,
    ...
    VLOG(INFO, "CHROMA IDC : %d\n", chroma_format_idx);
    if (NULL != ctx->comparator) {
        if (chroma_format_idx == 0) { // is monochrome
            Comparator_Configure(ctx->comparator, COMPARATOR_CONF_SET_MONOCHROME, NULL);
        }
    }
}
...
void HandleDecRegisterFbEvent(Component com, CNMComListenerDecRegisterFb* param, DecListenerContext* ctx)
{
    ...
    if ( ctx->performance ) {
        ctx->pfCtx = PFMonitorSetup(VPU_HANDLE_CORE_INDEX(param->handle), param->handle->instIndex, ctx->pfClock, fps, GetBasename((const char *)ctx->inputPath), 0);
    }
}
...
void HandleDecInterruptEvent(Component com, CNMComListenerDecInt* param, DecListenerContext* ctx)
{
    if ((1<<INT_WAVE5_DEC_PIC) == param->flag) {
        if (ctx->bwCtx != NULL) {
            BWMonitorUpdate(ctx->bwCtx, ctx->numVCores, BW_PARAM_NOT_USED);
...
void HandleDecGetOutputEvent(Component com, CNMComListenerDecDone* param, DecListenerContext* ctx)
{
    ...
    Uint32                  errReason;
    const DecInfo*          pDecInfo = NULL;
    if (param->ret != RETCODE_SUCCESS) return;

    pDecInfo = VPU_HANDLE_TO_DECINFO(param->handle);

    if (!(output->decodingSuccess & 0x01)) {
        errReason = output->errorReason;
        switch (errReason) {
        case WAVE5_SYSERR_FATAL_VPU_HANGUP:
        ...
        default:
            VLOG(WARN, "VPU_DecGetOutputInfo decode fail framdIdx %d error(0x%08x) reason(0x%08x), reasonExt(0x%08x)\n",
                param->numDecoded, output->decodingSuccess, output->errorReason, output->errorReasonExt);
        }
    }
    ...
    if (ctx->pfCtx != NULL) {
        if (output->indexFrameDecoded >= 0) {
            PFMonitorUpdate(VPU_HANDLE_CORE_INDEX(param->handle), ctx->pfCtx, output->frameCycle, output->seekCycle, output->parseCycle, output->DecodedCycle);
        }
    }

    if (output->indexFrameDecoded >= 0) {
        if (ctx->bwCtx != NULL) {
            BWMonitorUpdatePrint(ctx->bwCtx, output->picType, output->indexFrameDecoded, param->numDecoded);
        }
    }
...
#if defined(WAVE517) && defined(SUPPORT_VCPU_FPGA)
        if (3 == ctx->compareType) {
            if (FALSE == ComparePrescanData(ctx->inputPath, param->vcoreData)) {
                ctx->match = FALSE;
                CNMAppStop();
            }
        }
#endif /* WAVE517 && SUPPORT_VCPU_FPGA */
    }
    else {
        ctx->notDecodedCount++;
    ...
    if (ctx->productId == PRODUCT_ID_517 && output->sequenceChanged != 0) {
        VLOG(INFO, "Sequence changed CHROMA IDC : %d\n", pDecInfo->newSeqInfo.chromaFormatIDC);
        if (pDecInfo->newSeqInfo.chromaFormatIDC == 0) {
            if (ctx->comparator != NULL) {
                Comparator_Configure(ctx->comparator, COMPARATOR_CONF_SET_MONOCHROME, NULL);
            }
        } else {
            if (ctx->comparator != NULL) {
                Comparator_Configure(ctx->comparator, COMPARATOR_CONF_SET_NOT_MONOCHROME, NULL);
            }
        }
    }
    ...
    if (ctx->compareType == YUV_COMPARE) {
        rcDisplay.right = (STD_VP9 == ctx->bitFormat ? VPU_ALIGN8(output->dispPicWidth) : output->dispPicWidth);
        rcDisplay.bottom = (STD_VP9 == ctx->bitFormat ? VPU_ALIGN8(output->dispPicHeight) : output->dispPicHeight);
        if (ctx->enableScaler == TRUE) {
           rcDisplay.right  = VPU_CEIL(rcDisplay.right, 2);
           rcDisplay.bottom = VPU_CEIL(rcDisplay.bottom, 2);
        }
        if (TRUE == PRODUCT_ID_W_SERIES(ctx->productId)) {
            pYuv = GetYUVFromFrameBuffer(param->handle, &output->dispFrame, rcDisplay, &width, &height, &Bpp, &frameSizeInByte, FALSE);
        }
        decodedData     = (void*)pYuv;
    }

    if (ctx->comparator) {
        if ((ctx->match=Comparator_Act(ctx->comparator, decodedData, frameSizeInByte, param->handle->instIndex)) == FALSE) {
        }
    }
...
BOOL SetupDecListenerContext(DecListenerContext* ctx, CNMComponentConfig* config, Component renderer)
{
    ...
    ctx->scale_width = decConfig->scaleDownWidth;
    ctx->scale_height = decConfig->scaleDownHeight;
