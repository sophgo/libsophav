...
#define INDEX_FRAME_DIS_QUEUE_SIZE 128
...
static DEC_INT_STATUS HandlingInterruptFlagNoCQ(ComponentImpl* com, InterruptBit waitInterrptFlag)
{
    ...
    Uint32               interruptWaitTime = VPU_DEC_TIMEOUT;
...
static BOOL Decode(ComponentImpl* com, PortContainerES* in, PortContainerDisplay* out)
{
    ...
    if (TRUE == ctx->ppuFbParam.enablePPU) {
        if (decOutputInfo.indexFrameDisplay >= 0) {
            Queue_Enqueue(ctx->idxFrameDisQ, &(decOutputInfo));
    ...
    if (out->reuse == FALSE) {
        ...
        out->scaleX = lsnpPicDone.scaleX;
        out->scaleY = lsnpPicDone.scaleY;
        if (ctx->ppuFbParam.enablePPU) {
            if (1 < ctx->idxFrameDisQ->count || DISPLAY_IDX_FLAG_SEQ_END == out->decInfo.indexFrameDisplay) {
                if (NULL != Queue_Peek(ctx->idxFrameDisQ)) {
                    out->decInfo = *((DecOutputInfo*)Queue_Dequeue(ctx->idxFrameDisQ));
                }
            }
        }
    }
...
static CNMComponentParamRet GetParameterDecoder(ComponentImpl* from, ComponentImpl* com, GetParameterCMD commandType, void* data)
{
    ...
    case GET_PARAM_DEC_BITSTREAM_BUF_POS:
        ...
        } else if (ctx->state > DEC_STATE_DECODING) {
...
static CNMComponentParamRet SetParameterDecoder(ComponentImpl* from, ComponentImpl* com, SetParameterCMD commandType, void* data)
{
    ...
    case SET_PARAM_DEC_CLR_DISP:
        if (ctx->state == DEC_STATE_DECODING) {
            VPU_DecClrDispFlag(ctx->handle, *(int*)data);
        }
        break;
    case SET_PARAM_COM_STOP:
        stop = TRUE;
        ComponentNotifyListeners(com, COMPONENT_EVENT_DEC_DECODED_ALL, (void*)ctx->handle);
        ComponentSetParameter(com, com->sinkPort.connectedComponent, SET_PARAM_COM_STOP, &stop);
        ComponentSetParameter(com, com->srcPort.connectedComponent, SET_PARAM_COM_STOP, &stop);
        com->terminate = TRUE;
        break;
    default:
        result = FALSE;
...
static BOOL OpenDecoder(ComponentImpl* com)
{
    ...
    OpenDecReport(ctx->handle, &decReportCfg);
...
static BOOL DestroyDecoder(ComponentImpl* com)
{
    ...
    if (RETCODE_SUCCESS != ret) {
        VLOG(ERR, "%s:%d FAILED TO VPU_DecClose() ret(%d), inst=%d \n", __FUNCTION__, __LINE__, ret, VPU_HANDLE_INSTANCE_NO(ctx->handle));
    }
...
static Component CreateDecoder(ComponentImpl* com, CNMComponentConfig* componentParam)
{
    ...
    ctx->idxFrameDisQ = Queue_Create(INDEX_FRAME_DIS_QUEUE_SIZE, sizeof(DecOutputInfo));
