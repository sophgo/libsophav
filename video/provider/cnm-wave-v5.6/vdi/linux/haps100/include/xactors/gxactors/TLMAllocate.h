...
// DO NOT EDIT
// C++ Class with SceMi Message passing for type:  TLM3Defines::TLMAllocate

#pragma once

#include "bxactors/xat_scemi.h"

/// C++ class representing the hardware enum TLM3Defines::TLMAllocate.
class TLMAllocate : public XATType {
 public:
  enum E_TLMAllocate {
      e_NO_ALLOCATE,
      e_ALLOCATE
  ...
  TLMAllocate (E_TLMAllocate v = e_NO_ALLOCATE)
  : m_val(v)
  {}
   ...
   */
  TLMAllocate (const SceMiMessageDataInterface *msg, unsigned int & off) {
    m_val = (E_TLMAllocate) msg->GetBitRange ( off, 0 );
    off = off + 1;
  }
  ...
  bool operator== (const E_TLMAllocate &x) const { return m_val == x ;}
  /// overload operator != for enumeration classes with values
  bool operator!= (const E_TLMAllocate &x) const { return m_val != x ;}
  /// overload operator == for enumeration classes
  bool operator== (const TLMAllocate &x) const { return m_val == x.m_val ;}
  /// overload operator != for enumeration classes
  bool operator!= (const TLMAllocate &x) const { return m_val != x.m_val ;}

  /// overload the put-to operator for TLMAllocate
  friend std::ostream & operator<< (std::ostream &os, const TLMAllocate & e) {
    XATType::PutTo * override = lookupPutToOverride ( e.getClassName() );
    if ( override != 0 ) {
       return override(os, e );
    }
    switch (e.m_val) {
      case e_NO_ALLOCATE: os << "NO_ALLOCATE" ; break ;
      case e_ALLOCATE: os << "ALLOCATE" ; break ;
      default: os << "Enum value error for TLMAllocate: " << (int) e.m_val << "." ;
    }
    return os;
  };
  ...
  const char * getName() const {
    const char *ret = "" ;
    switch (m_val) {
      case e_NO_ALLOCATE: ret = "NO_ALLOCATE" ; break ;
      case e_ALLOCATE: ret = "ALLOCATE" ; break ;
      default: std::cerr << "Enum value error for TLMAllocate: " << (int) m_val << "." ;
    }
    return ret;
  };
