...
int md5_cmp(unsigned char* got, unsigned char* exp ,int size){
    ...
    md5_get(got, size, md5_str, 1);

    if (memcmp(md5_str, exp, MD5_STRING_LENGTH) != 0) {
        fprintf(stderr, "md5 cmp fail\n");
        return -1;
    } else {
        fprintf(stderr, "md5 cmp successful\n");
    }
    return 0;
}