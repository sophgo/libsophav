...
bm_status_t bmcv_faiss_indexflatL2(bm_handle_t handle,
    ...
    if (tpu_env == NULL) {
        bmlib_log("FAISS_INDEXFLATL2", BMLIB_LOG_DEBUG, "Use TPU core0\n");
    } else {
        if (strcmp(tpu_env, "0") == 0) {
            bmlib_log("FAISS_INDEXFLATL2", BMLIB_LOG_DEBUG, "Use TPU core0\n");
        } else if (strcmp(tpu_env, "1") == 0) {
            bmlib_log("FAISS_INDEXFLATL2", BMLIB_LOG_DEBUG, "Use TPU core1\n");
            if_core0 = 0;
            if_core1 = 1;
        } else if (strcmp(tpu_env, "2") == 0 || strcmp(tpu_env, "both") == 0) {
            bmlib_log("FAISS_INDEXFLATL2", BMLIB_LOG_DEBUG, "Use all TPU cores (0 and 1)\n");
            if_core1 = 1;
        } else {
            bmlib_log("FAISS_INDEXFLATL2", BMLIB_LOG_ERROR, "Invalid TPU_CORES value: %s\n", tpu_env);
            bmlib_log("FAISS_INDEXFLATL2", BMLIB_LOG_ERROR, "Available options: 0, 1, 2/both\n");
            exit(EXIT_FAILURE);
        }
    }
    ret = bmcv_faiss_indexflatL2_check(database_vecs_num, sort_cnt, query_vecs_num, input_dtype, output_dtype, is_transpose);
    if (ret != BM_SUCCESS) {
        printf("bmcv_faiss_indexflatl2_check failed!\n");
        return ret;
    }
