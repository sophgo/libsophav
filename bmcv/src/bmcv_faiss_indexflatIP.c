...
bm_status_t bmcv_faiss_indexflatIP(bm_handle_t handle,
    ...
    if(if_core0 == 1 && if_core1 == 1) {
        ...
        sg_api_indexflatIP_dual_core_t params[BM1688_MAX_CORES];
        for (int core_idx = 0; core_idx < core_nums; core_idx++) {
            ...
            launch_params[core_idx].param_size = sizeof(sg_api_indexflatIP_dual_core_t);
        }
        switch (chipid) {
            case BM1688_PREV:
            case BM1688:
                ret = bm_tpu_kernel_launch_dual_core(handle, "faiss_api_indexflatIP_bm1688", launch_params, core_list, core_nums);
                if (BM_SUCCESS != ret) {
        ...
        if(query_vecs_num == 1) {
            ...
            int *dual_core_similarity_host_int, *similarity_host_int;
            ...
            if (output_dtype == DT_FP32) {
                ...
                find_topk_fp32(dual_core_similarity_host_fp32, dual_core_index_host,
                               similarity_host_fp32, index_host, sort_cnt, 2, 1);
                ...
                find_topk_fp16(dual_core_similarity_host_fp16, dual_core_index_host,
                                similarity_host_fp16, index_host, sort_cnt, 2, 1);
                BM_CHECK_RET(bm_memcpy_s2d(handle, output_sorted_similarity_global_addr,
                ...
                dual_core_similarity_host_int = (int *)malloc(sizeof(int) * query_vecs_num * sort_cnt * 2);
                similarity_host_int = (int *)malloc(sizeof(int) * query_vecs_num * sort_cnt);
                BM_CHECK_RET(bm_memcpy_d2s(handle, bm_mem_get_system_addr(bm_mem_from_system(dual_core_similarity_host_int)),
                             output_dual_core_sorted_similarity_global_addr));
                find_topk_int(dual_core_similarity_host_int, dual_core_index_host,
                                similarity_host_int, index_host, sort_cnt, 2);
                BM_CHECK_RET(bm_memcpy_s2d(handle, output_sorted_similarity_global_addr,
                             bm_mem_get_system_addr(bm_mem_from_system(similarity_host_int))));
                free(dual_core_similarity_host_int);
                free(similarity_host_int);
            }
    ...
    if(if_core0 == 1 && if_core1 == 1) {
        ...
        if(query_vecs_num == 1) {
            ...
            if (output_dtype == DT_FP32) {
                ...
                dual_core_similarity_host_int = (int *)malloc(sizeof(int) * query_vecs_num * sort_cnt * 2);
                similarity_host_int = (int *)malloc(sizeof(int) * query_vecs_num * sort_cnt);
                BM_CHECK_RET(bm_memcpy_d2s_u64(handle, bm_mem_get_system_addr(bm_mem_from_system(dual_core_similarity_host_int)),
                             output_dual_core_sorted_similarity_global_addr));
                find_topk_int(dual_core_similarity_host_int, dual_core_index_host,
                                similarity_host_int, index_host, sort_cnt, 2);
                BM_CHECK_RET(bm_memcpy_s2d_u64(handle, output_sorted_similarity_global_addr,
