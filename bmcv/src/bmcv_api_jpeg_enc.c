...
#define JPEG_ENC_LOG_TAG "JPEG-ENC"
...
/*
        bmlib_log(JPEG_ENC_LOG_TAG, BMLIB_LOG_WARNING,
...
static bm_status_t bmcv_jpeg_enc_check(bm_image *src, int image_num, int quality_factor) {
    if (NULL == src) {
        bmlib_log(JPEG_ENC_LOG_TAG, BMLIB_LOG_ERROR,
...
static int bmcv_jpeg_encoder_create(bmcv_jpeg_encoder_t** p_jpeg_encoder,
    ...
    if (BM_JPU_ENC_RETURN_CODE_OK != ret) {
        bmlib_log(JPEG_ENC_LOG_TAG, BMLIB_LOG_ERROR, "load jpeg encoder failed!\r\n");
        goto End;
    }
...
static int bmcv_jpeg_encoder_destroy(bmcv_jpeg_encoder_t *jpeg_encoder)
{
    ...
    bm_jpu_enc_unload(jpeg_encoder->dev_id);
    free(jpeg_encoder);
    return 0;
}
...
bm_status_t bmcv_jpeg_enc_one_image(bmcv_jpeg_encoder_t  *jpeg_enc,
    ...
    if (ret != BM_JPU_ENC_RETURN_CODE_OK) {
        bmlib_log(JPEG_ENC_LOG_TAG, BMLIB_LOG_ERROR, "jpeg encode failed!\r\n");
...
bm_status_t bmcv_image_jpeg_enc(bm_handle_t  handle,
    ...
    if (handle == NULL) {
        bmlib_log(JPEG_ENC_LOG_TAG, BMLIB_LOG_ERROR, "Can not get handle!\r\n");
        return BM_ERR_FAILURE;
    }
    ...
    if (out_size == NULL) {
        bmlib_log(JPEG_ENC_LOG_TAG, BMLIB_LOG_ERROR, "pointer of out size is NULL,  \
    ...
    for (int i = 0; i < image_num; i++) {
        ...
        if (src_is_align) {
            ...
            if (ret != BM_SUCCESS) {
                bmlib_log(JPEG_ENC_LOG_TAG, BMLIB_LOG_ERROR, "internal creat bm_image failed!\n");
                for (int j = 0; j < i; j++) {
                    free(p_jpeg_data[j]);
                }
                free(out_size);
            ...
            if (ret != BM_SUCCESS) {
                bmlib_log(JPEG_ENC_LOG_TAG, BMLIB_LOG_ERROR, "internal width align failed!\n");
                for (int j = 0; j < i; j++) {
                    free(p_jpeg_data[j]);
                }
                free(out_size);
