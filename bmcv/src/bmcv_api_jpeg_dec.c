...
static bm_status_t bmcv_jpeg_dec_check(bm_image*        src,
    ...
    if (src->data_type != DATA_TYPE_EXT_1N_BYTE) {
        bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_ERROR,
...
static int bmcv_jpeg_decoder_create(bmcv_jpeg_decoder_t** p_jpeg_decoder,
    ...
    if (dec == NULL) {
        bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_ERROR, "internal malloc failed!\r\n");
    }
    ...
    switch (dst->image_format) {
        ...
        case FORMAT_GRAY:
            ...
            break;
        case FORMAT_YUV444P:
            open_params.color_format = BM_JPU_COLOR_FORMAT_YUV444;
            break;
        default:
            bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_WARNING, "unknown image format %d\r\n", dst->image_format);
    ...
    if (BM_JPU_DEC_RETURN_CODE_OK != ret) {
        bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_ERROR, "load jpeg decoder failed!\r\n");
        goto End;
    }

    ret = bm_jpu_jpeg_dec_open(&dec->decoder_, &open_params, 0);
    if (BM_JPU_DEC_RETURN_CODE_OK != ret) {
        bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_ERROR, "open jpeg decoder failed!\r\n");
       goto End;
    }
...
static int bmcv_jpeg_decoder_destroy(bmcv_jpeg_decoder_t *jpeg_decoder)
{
    ...
    bm_jpu_dec_unload(jpeg_decoder->dev_id);
    free(jpeg_decoder);
    return 0;
}
...
static int check_jpeg_baseline(struct jpeg_decompress_struct* cinfo)
{
...
static int determine_hw_decoding(struct jpeg_decompress_struct* cinfo)
{
                           ...
                           bm_image     *dst)
{
    ...
    if (!determine_hw_decoding(&cinfo)){
        ...
        soft_decoding = 1;
        bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_INFO, "use soft jpeg decoding!\r\n");
        ...
        if (dst == NULL){
            ...
            if (dst->data_type != DATA_TYPE_EXT_1N_BYTE) {
                bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_ERROR, "data type only support 1N_BYTE %s: %s: %d\n",
                    filename(__FILE__), __func__, __LINE__);
                soft_decoding = -BM_NOT_SUPPORTED;
                goto END;
            }
            ...
            if (dst->image_format == FORMAT_GRAY) cinfo.out_color_space = JCS_GRAYSCALE;
                ...
                bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_ERROR, "unsupported format %s: %s: %d: %d\n",
        ...
        if (!bm_image_is_attached(*dst)){
            ...
            if (BM_SUCCESS != bm_image_alloc_dev_mem(*dst, BMCV_HEAP1_ID)) {
...
bm_status_t bmcv_jpeg_dec_one_image(bm_handle_t          handle,
    ...
    bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_INFO,
    ...
    {
        bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_ERROR, "could not decode this JPEG image : no framebuffer returned!\r\n");
        return BM_ERR_FAILURE;
    }

    u64 phys_addr = bm_mem_get_device_addr(*(info.framebuffer->dma_buffer));
...
bm_status_t bmcv_image_jpeg_dec(bm_handle_t  handle,
                                ...
                                bm_image*    dst) {
    if (handle == NULL) {
        bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_ERROR, "Can not get handle!\r\n");
        return BM_ERR_FAILURE;
    }
    if (in_size == NULL || p_jpeg_data == NULL) {
        bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_ERROR, "The pointer of data and size should not be NULL!\r\n");
        return BM_ERR_FAILURE;
    }
    for (int i = 0; i < image_num; i++) {
        if (p_jpeg_data[i] == NULL) {
            bmlib_log(JPEG_DEC_LOG_TAG, BMLIB_LOG_ERROR, "The pointer of data should not be NULL!\r\n");
            return BM_ERR_FAILURE;
        }
    }
