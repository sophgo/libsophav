...
char *dst_name = "16bitTo8bit_res.bin";
...
static void *ive_16bitTo8bit(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            ...
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive 16bitTo8bit] cmp_md5 failed, cmp = %d \n", cmp);
                bm_image_destroy(&src);
                bm_image_destroy(&dst);
                free(output_ptr);
                exit(-1);
...
int main(int argc, char **argv) {
    ...
    if ((argc > 3 && argc < 10) || (argc == 1)) {
        ...
        %s width height mode u8Numerator u16Denominator s8Bias srcDtype dstDtype src_name ref_name dev_id thread_num loop_num bWrite dst_name\n \
