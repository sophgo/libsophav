...
bm_status_t vpss_fill_rectangle(unsigned char *src_data,
                                int in_width,
                                int in_height,
    ...
    if (ret != BM_SUCCESS) {
        printf("bmcv_image_fill_rectangle failed! \n");
    }
    ...
    printf("Fill_rectangle VPSS using time: %llu(us), fps : %llu, %lluM pps\n", using_time, fps_actual, pixel_per_sec);
...
bm_status_t cmodel_fill_rectangle(unsigned char *src_data,
    ...
    coverex_cfg my_coverex = {
        .coverex_param = {},
        .cover_num = rect_num};
    for (int i = 0; i < my_coverex.cover_num; i++) {
        my_coverex.coverex_param[i].enable = 1;
        my_coverex.coverex_param[i].rect.left = crop_rect[i].start_x;
        my_coverex.coverex_param[i].rect.top = crop_rect[i].start_y;
        my_coverex.coverex_param[i].rect.width = crop_rect[i].crop_w;
        my_coverex.coverex_param[i].rect.height = crop_rect[i].crop_h;
        my_coverex.coverex_param[i].color = ((r << 16) | (g << 8) | (b));
    }
    ...
    printf("Fill_rectangle CPU using time: %d(us)\n", using_time);
...
bm_status_t vpss_fill_rectangle_cmp(int use_real_img,
                                    char *filename_src,
                                    char *filename_dst,
                                    int in_width,
                                    int in_height,
    ...
    printf("===== test vpss fill_rectangle =====\n");
    ...
    vpss_fill_rectangle(vpss_output, in_width, in_height, src_format, rect_num, crop_rect, r, g, b);
    // dump output
    ...
    cmodel_fill_rectangle(input_data, cmodel_output, in_width, in_height, src_format, rect_num, crop_rect, r, g, b);
    // dump output
    if (filename_dst[0] != '\0' && filename_src[0] != '\0') {
...
int main(int argc, char **argv) {
    ...
    if (argc != 13 && argc != 14 && argc != 15) {
        printf("usage: %s in_w in_h src_format rect_num(1) start_x start_y crop_x crop_y  r_value g_value b_value use_real_img (src.bin) (dst.bin)\n", argv[0]);
        printf("example:\n");
        printf("%s 1920 1080 8 1 480 270 960 540  0 255 0 0\n", argv[0]);
        printf("%s 1920 1080 8 1 480 270 960 540  0 255 0 1 res/1920x1080_rgbp.bin\n", argv[0]);
        printf("%s 1920 1080 8 1 480 270 960 540  0 255 0 1 res/1920x1080_rgbp.bin output.bin\n", argv[0]);
        return -1;
    }
    ...
    if (ret != BM_SUCCESS) {
        printf("Compare vpss_fill_rectangle result with cmodel failed! \n");
    } else {
        printf("Compare vpss_fill_rectangle result with cmodel successfully ! \n");
    }
    return ret;
}
