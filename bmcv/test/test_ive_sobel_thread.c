    ...
    if (ctx.i == 0) {
        ...
        if (enMode == BM_IVE_SOBEL_OUT_MODE_BOTH || enMode == BM_IVE_SOBEL_OUT_MODE_HOR) {
            ...
            if (strchr(golden_hDstName, '.') != NULL) {
                ...
                if (cmp != 0) {
                    printf("[bmcv ive sobel] sobelResult_h cmp_s16 failed, cmp = %d \n", cmp);
                    free(iveSobel_h);
                    free(iveSobel_v);
                    exit(-1);
                }
                printf("[bmcv ive sobel] sobelResult_h cmp_s16 successful, cmp = %d \n", cmp);
            }
            else {
                int cmp = md5_cmp((unsigned char*)iveSobel_h, (unsigned char*)golden_hDstName, width * height * sizeof(signed short));
                if (cmp != BM_SUCCESS) {
                    printf("[bmcv ive sobel] cmp_md5 failed, cmp = %d \n", cmp);
                    free(iveSobel_h);
                    free(iveSobel_v);
                    exit(-1);
                }
        ...
        if (enMode == BM_IVE_SOBEL_OUT_MODE_BOTH || enMode == BM_IVE_SOBEL_OUT_MODE_VER) {
            ...
            if (strchr(golden_vDstName, '.') != NULL) {
                ...
                if (cmp != 0) {
                    printf("[bmcv ive sobel] sobelResult_v cmp_s16 failed, cmp = %d \n", cmp);
                    free(iveSobel_h);
                    free(iveSobel_v);
                    exit(-1);
                }
                printf("[bmcv ive sobel] sobelResult_v cmp_s16 successful, cmp = %d \n", cmp);
            }
            else {
                int cmp = md5_cmp((unsigned char*)iveSobel_v, (unsigned char*)golden_vDstName, width * height * sizeof(signed short));
                if (cmp != BM_SUCCESS) {
                    printf("[bmcv ive sobel] cmp_md5 failed, cmp = %d \n", cmp);
                    free(iveSobel_h);
                    free(iveSobel_v);
