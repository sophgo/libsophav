...
char *src_name = "imgL_1280X720.yonly.yuv", *dst_name = "out_gdc_grid_L.yuv", *grid_name = "grid_info_79_43_3397_80_45_1280x720.dat";
bm_handle_t handle = NULL;
bmcv_gdc_attr ldc_attr = {0};
char *md5 = "ad2ec6fe09dea2b4c7163b62c0fad5ce";

static void * dwa_gdc_grid_info(void* arg) {
    bm_status_t ret;
    ...
    ldc_attr.grid_info.u.system.system_addr = (void *)ctx.buffer;
    ...
    printf("idx:%d, bmcv_dwa_gdc_grid_info:loop %d cycles, time_max = %llu, time_avg = %llu, fps %llu, %lluM pps\n",
...
static void print_help(char **argv){
    printf("please follow this order:\n \
        %s src_w src_h src_fmt src_name dst_name grid_file_name thread_num loop_num md5\n \
...
int main(int argc, char **argv) {
    ...
    if (argc >= 8) {
        ...
        dst_name = argv[5];
        grid_name = argv[6];
        ldc_attr.grid_info.size = atoi(argv[7]);
    }
    if (argc == 2){
        if (atoi(argv[1]) < 0){
            print_help(argv);
    ...
    for (int i = 0; i < test_threads_num; i++) {
        ...
        ctx[i].buffer = (char *)malloc(ldc_attr.grid_info.size);
        if (ctx[i].buffer == NULL) {
            printf("malloc buffer for grid_info failed!\n");
            exit(-1);
        }
        memset(ctx[i].buffer, 0, ldc_attr.grid_info.size);
        ...
        fread(ctx[i].buffer, 1, ldc_attr.grid_info.size, fp);

        fclose(fp);
        if (pthread_create(
                &pid[i], NULL, dwa_gdc_grid_info, (void *)(ctx + i))) {
            free(pid);
