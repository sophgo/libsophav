...
static void *ive_ccl(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            ...
            printf("[bmcv ive ccl] ccblob cmp successful \n");
        }
        else {
            int cmp = md5_cmp((unsigned char*)ccblob, (unsigned char*)ref_name, sizeof(bmcv_ive_ccblob));
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive ccl] cmp_md5 failed, cmp = %d \n", cmp);
                free(ccblob);
                bm_image_destroy(&src_dst_img);
                bm_free_device(handle, pst_blob);
                exit(-1);
