...
static int bmcv_warp_tpu(bm_handle_t handle,
    ...
    bm_image                 src_img[4];
    bm_image_format_ext      image_format = FORMAT_GRAY;//FORMAT_GRAY FORMAT_BGR_PLANAR
    ...
    if (!use_opencv) {
        ...
        ret = bmcv_image_warp_affine_padding(handle, image_n, matrix_image, src_img, dst_img, is_bilinear);
        ...
        printf("opencv mode\n");
        gettimeofday(&t1, NULL);
        ret = bmcv_image_warp_affine_similar_to_opencv_padding(handle, image_n, matrix_image, src_img, dst_img, is_bilinear);
        gettimeofday(&t2, NULL);
        if (BM_SUCCESS != ret) {
            free(dst_img);
            return -1;
