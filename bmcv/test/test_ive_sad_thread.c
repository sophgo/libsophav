...
void md5_cmp_be(unsigned char* got_md5, unsigned char* got, int w, int h, int dsize, bool isDmaHalf) {
    int stride = align_up(width, 16);
    int sad_stride = (isDmaHalf) ? stride / 2 : stride;
    for (int i = 0; i < h; i++, got += sad_stride * dsize, got_md5 += w * dsize)
        memcpy(got_md5, got, w * dsize);
}
...
static void *ive_sad(void* arg) {
    ...
    if (sadOutCtrl == BM_IVE_SAD_OUT_BOTH || sadOutCtrl == BM_IVE_SAD_OUT_SAD || sadOutCtrl == BM_IVE_SAD_OUT_THRESH) {
    ...
    if (ctx.i == 0) {
        ...
        if (sadOutCtrl == BM_IVE_SAD_OUT_BOTH || sadOutCtrl == BM_IVE_SAD_OUT_SAD) {
            ret = bm_image_copy_device_to_host(dstSad, (void **)&sad_res);
            if (ret != BM_SUCCESS) {
                printf("bm_image_copy_device_to_host failed. ret = %d\n", ret);
                exit(-1);
            }
            if (strchr(refSad_name, '.') != NULL) {
                int cmp = cmp_sad(refSad_name, sad_res, dst_width, dst_height, data_size, datatype);
                if (cmp != 0) {
                    printf("[bmcv ive sad] sad_res cmp_u8 failed, cmp = %d \n", cmp);
                    exit(-1);
                }
                printf("[bmcv ive sad] sad_res cmp_u8 successful, cmp = %d \n", cmp);
            }
            else {
                unsigned char *sad_got_md5 = (unsigned char*)malloc(dst_width * dst_height * data_size);
                memset(sad_got_md5, 0, dst_width * dst_height * data_size);
                md5_cmp_be(sad_got_md5, sad_res, dst_width, dst_height, data_size, datatype);
                int cmp = md5_cmp((unsigned char*)sad_got_md5, (unsigned char*)refSad_name, dst_width * dst_height * data_size);
                if (cmp != BM_SUCCESS) {
                    printf("[bmcv ive sad] sad_res cmp_md5 failed, cmp = %d \n", cmp);
                    exit(-1);
                }
                free(sad_got_md5);
            }
            if (bWrite) {
                int sad_stride = (dst_sad_dtype == DATA_TYPE_EXT_1N_BYTE) ? dstSad_stride[0] / 2 : dstSad_stride[0];
                sad_write_img(dstSad_name, sad_res, width, height, data_size, sad_stride);
            }
        }
        if (sadOutCtrl == BM_IVE_SAD_OUT_BOTH || sadOutCtrl == BM_IVE_SAD_OUT_THRESH) {
            ...
            if (strchr(refSad_name, '.') != NULL) {
                int cmp = cmp_sad(refThr_name, thr_res, dst_width, dst_height, sizeof(unsigned char), false);
                if (cmp != 0) {
                    printf("[bmcv ive sad] thr_res cmp_u8 failed, cmp = %d \n", cmp);
                    exit(-1);
                }
                printf("[bmcv ive sad] thr_res cmp_u8 successful, cmp = %d \n", cmp);
            }
            else {
                unsigned char *thr_got_md5 = (unsigned char*)malloc(dst_width * dst_height * sizeof(unsigned char));
                memset(thr_got_md5, 0, dst_width * dst_height * sizeof(unsigned char));
                md5_cmp_be(thr_got_md5, thr_res, dst_width, dst_height, sizeof(unsigned char), false);
                int cmp = md5_cmp((unsigned char*)thr_got_md5, (unsigned char*)refThr_name, dst_width * dst_height * sizeof(unsigned char));
                if (cmp != BM_SUCCESS) {
                    printf("[bmcv ive sad] thr_res cmp_md5 failed, cmp = %d \n", cmp);
                    exit(-1);
                }
                free(thr_got_md5);
            }
            if (bWrite)
                sad_write_img(dstThr_name, thr_res, width, height, sizeof(unsigned char), stride[0]);
        }
        free(sad_res);
        free(thr_res);
    }
...
int main(int argc, char **argv) {
    ...
    if (argc == 2)
        ...
        %s width height sad_mode sad_outCtrl_mode datatype sadThr sadMinValue sadMaxValue src1_name src2_name refSad_name refThr_name dev_id thread_num loop_num bWrite dstSad_name dstSadThr_name\n \
