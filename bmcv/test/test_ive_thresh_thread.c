...
typedef struct ive_thresh_ctx_ {
    int loop;
    int i;
} ive_thresh_ctx;
...
char *ref_name = "./ive_data/result/sample_Thresh_MinMidMax.yuv";
...
static void *ive_thresh(void* arg) {
    bm_status_t ret;
    ive_thresh_ctx ctx = *(ive_thresh_ctx*)arg;
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive thresh] cmp_u8 failed, cmp = %d \n", cmp);
                exit(-1);
            }
            printf("[bmcv ive thresh] cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp((unsigned char*)ive_res, (unsigned char*)ref_name, width * height * sizeof(unsigned char));
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive thresh] cmp_md5 failed, cmp = %d \n", cmp);
                exit(-1);
            }
        }
        if (bWrite) {
    ...
    char algorithm_str[100] = "ive_thresh";
    ...
    printf("idx:%d, bm_ive_thresh: loop %d cycles, time_max = %llu, time_avg = %llu, fps %llu, %lluM pps\n",
...
int main(int argc, char **argv) {
    ...
    if (test_loop_times > 15000 || test_loop_times < 1) {
        printf("[TEST ive thresh] loop times should be 1~15000\n");
        exit(-1);
    }
    if (test_threads_num > MAX_THREAD_NUM || test_threads_num < 1) {
        printf("[TEST ive thresh] thread nums should be 1~%d\n", MAX_THREAD_NUM);
        exit(-1);
    }
    ...
    ive_thresh_ctx ctx[MAX_THREAD_NUM];
#ifdef __linux__
    ...
    for (int i = 0; i < test_threads_num; i++) {
        ...
        if (pthread_create(
                    &pid[i], NULL, ive_thresh, (void *)(ctx + i))) {
