...
static void *ive_normgrad(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (enMode == BM_IVE_NORM_GRAD_OUT_HOR_AND_VER || enMode == BM_IVE_NORM_GRAD_OUT_HOR) {
            ...
            if (strchr(golden_hDstName, '.') != NULL) {
                int cmp = cmp_s8(golden_hDstName, ive_h_res, width * height * sizeof(signed char));
                if (cmp != 0) {
                    printf("[bmcv ive ive_h_res] cmp_s8 failed, cmp = %d \n", cmp);
                    free(ive_h_res);
                    free(ive_v_res);
                    free(ive_combine_res);
                    exit(-1);
                }
                printf("[bmcv ive ive_h_res] cmp_s8 successful, cmp = %d \n", cmp);
            }
            else {
                int cmp = md5_cmp((unsigned char*)ive_h_res, (unsigned char*)golden_hDstName, width * height * sizeof(signed char));
                if (cmp != BM_SUCCESS) {
                    printf("[bmcv ive ive_h_res] cmp_md5 failed, cmp = %d \n", cmp);
                    free(ive_h_res);
                    free(ive_v_res);
                    free(ive_combine_res);
                    exit(-1);
        ...
        if (enMode == BM_IVE_NORM_GRAD_OUT_HOR_AND_VER || enMode == BM_IVE_NORM_GRAD_OUT_VER) {
            ...
            if (strchr(golden_vDstName, '.') != NULL) {
                int cmp = cmp_s8(golden_vDstName, ive_v_res, width * height * sizeof(signed char));
                if (cmp != 0) {
                    printf("[bmcv ive ive_v_res] cmp_s8 failed, cmp = %d \n", cmp);
                    free(ive_h_res);
                    free(ive_v_res);
                    free(ive_combine_res);
                    exit(-1);
                }
                printf("[bmcv ive ive_v_res] cmp_s8 successful, cmp = %d \n", cmp);
            }
            else {
                int cmp = md5_cmp((unsigned char*)ive_v_res, (unsigned char*)golden_vDstName, width * height * sizeof(signed char));
                if (cmp != BM_SUCCESS) {
                    printf("[bmcv ive ive_v_res] cmp_md5 failed, cmp = %d \n", cmp);
                    free(ive_h_res);
                    free(ive_v_res);
                    free(ive_combine_res);
                    exit(-1);
        ...
        if (enMode == BM_IVE_NORM_GRAD_OUT_COMBINE) {
            ...
            if (strchr(golden_hvDstName, '.') != NULL) {
                int cmp = cmp_u16(golden_hvDstName, ive_combine_res, width * height);
                if (cmp != 0) {
                    printf("[bmcv ive ive_combine_res] cmp_u16 failed, cmp = %d \n", cmp);
                    free(ive_h_res);
                    free(ive_v_res);
                    free(ive_combine_res);
                    exit(-1);
                }
                printf("[bmcv ive ive_combine_res] cmp_u16 successful, cmp = %d \n", cmp);
            }
            else {
                int cmp = md5_cmp((unsigned char*)ive_combine_res, (unsigned char*)golden_hvDstName, width * height * sizeof(unsigned short));
                if (cmp != BM_SUCCESS) {
                    printf("[bmcv ive ive_combine_res] cmp_md5 failed, cmp = %d \n", cmp);
                    free(ive_h_res);
                    free(ive_v_res);
                    free(ive_combine_res);
                    exit(-1);
