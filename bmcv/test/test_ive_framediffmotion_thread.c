...
char *dst_name = "frameDiffMotion_res.bin";
...
static void *ive_frameDiffMotion(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            ...
            int cmp = md5_cmp(ive_add_res, (unsigned char*)ref_name, width * height * sizeof(unsigned char));
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive framediffmotion] cmp_md5 failed, cmp = %d \n", cmp);
                exit(-1);
            }
            printf("[bmcv ive framediffmotion] cmp_md5 successful, cmp = %d \n", cmp);
        }
        free(ive_add_res);
    }
...
int main(int argc, char **argv) {
    ...
    if ((argc > 3 && argc < 6) || (argc == 1)) {
        ...
        %s width height src1_name src2_name ref_name dev_id thread_num loop_num bWrite dst_name\n \
