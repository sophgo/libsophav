...
static void *ive_bersen(void* arg) {
    ...
    if (ctx.i == 0) {
        unsigned char *ive_bernsen_res = (unsigned char*)malloc(width * height * sizeof(unsigned char));
        memset(ive_bernsen_res, 0, width * height * sizeof(unsigned char));
        ret = bm_image_copy_device_to_host(dst, (void **)&ive_bernsen_res);
        if (ret != BM_SUCCESS) {
            printf("dst bm_image_copy_device_to_host failed, ret is %d \n", ret);
            bm_image_destroy(&src);
            bm_image_destroy(&dst);
            free(ive_bernsen_res);
            exit(-1);
        }
        if (strchr(ref_name, '.') != NULL) {
            int cmp = cmp_u8(ref_name, ive_bernsen_res, width * height * sizeof(unsigned char));
            if (cmp != 0) {
            ...
            int cmp = md5_cmp(ive_bernsen_res, (unsigned char*)ref_name, width * height * sizeof(unsigned char));
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive bernsen] cmp_md5 failed, cmp = %d \n", cmp);
                bm_image_destroy(&src);
                bm_image_destroy(&dst);
                free(ive_bernsen_res);
                exit(-1);
        ...
        if (bWrite) bm_ive_write_bin(dst, dst_name);
        free(ive_bernsen_res);
    }
    bm_image_destroy(&src);
    bm_image_destroy(&dst);
...
int main(int argc, char **argv) {
    ...
    if ((argc > 3 && argc < 6) || (argc == 1)) {
        ...
        %s width height mode winsize src_name ref_name dev_id thread_num loop_num bWrite dst_name\n \
