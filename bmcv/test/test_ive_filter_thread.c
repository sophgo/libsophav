...
static void *ive_filter(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(goldenDst_name, '.') != NULL) {
            int cmp = cmp_u8(goldenDst_name, output_ptr, byte_size);
            if (cmp != 0) {
                printf("[bmcv ive filter] cmp_u8 failed, cmp = %d \n", cmp);
                bm_image_destroy(&src);
                bm_image_destroy(&dst);
                free(output_ptr);
                exit(-1);
            }
            printf("[bmcv ive filter] cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(output_ptr, (unsigned char*)goldenDst_name, byte_size);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive csc] cmp_md5 failed, cmp = %d \n", cmp);
                bm_image_destroy(&src);
                bm_image_destroy(&dst);
                free(output_ptr);
                exit(-1);
        ...
        if (bWrite) {
            ...
            fwrite((void *)output_ptr, 1, byte_size, filter_fp);
            fclose(filter_fp);
        }
        free(output_ptr);
    }
