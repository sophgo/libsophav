...
bmcv_dpu_online_mode dpu_online_mode = DPU_ONLINE_MUX0;
bmcv_dpu_disp_range disp_range = BMCV_DPU_DISP_RANGE_16;
bm_handle_t handle = NULL;

typedef struct dpu_online_t {
    int i_test;
} dpu_online_t;
...
static void *test_dpu_online(void* args) {
    dpu_online_t dpu_online_thread_arg = *(dpu_online_t*)args;
    ...
    (dpu_online_mode == DPU_ONLINE_MUX0) ? (output_dtype = DATA_TYPE_EXT_1N_BYTE) :
    ...
    if (ret != BM_SUCCESS) {
        printf("[Thread %d] Init online dpu failed, ret is %d \n", dpu_online_thread_arg.i_test, ret);
        exit(-1);
    }
    for (int i = 0; i < loop_time; i++) {
        gettimeofday(&tv_start, NULL);
        ret = bmcv_dpu_online_disp(handle, &left_img, &right_img, &disp_img, &dpu_sgbm_attr, &dpu_fgs_attr, dpu_online_mode);
        ...
        if (ret != BM_SUCCESS) {
            printf("[Thread %d] bmcv_dpu_online_disp failed. \n", dpu_online_thread_arg.i_test);
    ...
    if (read_from_bin) {
        printf("------Thread %d------ \nwidth         = %d \nheight        = %d \nloop          = %d \ndpu_online_mode  = %d \n", dpu_online_thread_arg.i_test, img_width, img_height, loop_time, dpu_online_mode);
    ...
    if (dpu_online_thread_arg.i_test == 0) {
        ...
        if (read_from_bin) {
            ...
            ret = cmp_md5(ref_res_path, u8_dpu_res, img_size, dpu_online_thread_arg.i_test);
            if (ret != BM_SUCCESS) {
            ...
            if (write_to_bin) {
                ...
                printf("[Thread %d] bm_write_bin write %s \n", dpu_online_thread_arg.i_test, dpu_res_path);
            }
            free(u8_dpu_res);
        }
        else {
            ...
            if (dpu_online_mode == DPU_ONLINE_MUX0) {
                ...
                ret = test_cmodel_online(left_img_cmodel, right_img_cmodel, cpu_res, left_img.width, left_img.height, dpu_online_mode, &dpu_sgbm_attr, &dpu_fgs_attr);
                if (ret != BM_SUCCESS) {
                    printf("[Thread %d] test_cmodel_online_u16 failed, ret = %d \n", dpu_online_thread_arg.i_test, ret);
                    free(cpu_res);
                    free(dpu_res);
                ...
                ret = cmp_u8(dpu_res, cpu_res, img_width, img_height, dpu_online_thread_arg.i_test);
                if (ret != BM_SUCCESS) {
                ...
                ret = test_cmodel_online_u16(left_img_cmodel, right_img_cmodel, cpu_res, left_img.width, left_img.height, dpu_online_mode, &dpu_sgbm_attr, &dpu_fgs_attr);
                if (ret != BM_SUCCESS) {
                    printf("[Thread %d] test_cmodel_online_u16 failed, ret = %d \n", dpu_online_thread_arg.i_test, ret);
                    free(cpu_res);
                    free(dpu_res);
                ...
                ret = cmp_u16(dpu_res, cpu_res, img_width, img_height, dpu_online_thread_arg.i_test);
                if (ret != BM_SUCCESS) {
...
int main(int argc, char **args) {
    ...
    dpu_online_mode = rand() % 3 + DPU_ONLINE_MUX0;
    bm_status_t ret = BM_SUCCESS;
    ...
    if (argc == 2 && atoi(args[1]) == -1) {
        ...
        printf("%s 1 1 128 128 /opt/sophon/libsophon-current/bin/res/128x128_sophgo.bin /opt/sophon/libsophon-current/bin/res/128x128_sophgo.bin /opt/sophon/libsophon-current/bin/res/128x128_sophgo.bin online_res.bin \n", args[0]);
        ...
        case 1 ... 5: {
            ...
            if ((img_width % 32) != 0) dpu_online_mode = DPU_ONLINE_MUX0;
            break;
        }
        case 9: {
        ...
        default:
            printf("[TEST dpu online] Thread nums should be 1~5,9 \n");
            exit(-1);
            break;
        }
    }
    dpu_online_t dpu_online_thread_arg[thread_num];
#ifdef __linux__
    pthread_t pid[thread_num];
    for (int i = 0; i < thread_num; i++) {
        dpu_online_thread_arg[i].i_test = i;
        if (pthread_create(pid + i, NULL, test_dpu_online, (void *)(dpu_online_thread_arg + i))) {
