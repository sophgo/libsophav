...
static void *ive_lbp(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(golden_sample_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive lbp] cmp failed, cmp = %d \n", cmp);
                free(ive_lbp_res);
                bm_image_destroy(&src);
                bm_image_destroy(&dst);
                exit(-1);
            }
            printf("[bmcv ive lbp] cmp successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(ive_lbp_res, (unsigned char*)golden_sample_name, width * height);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive lbp] cmp_md5 failed, cmp = %d \n", cmp);
                free(ive_lbp_res);
                bm_image_destroy(&src);
                bm_image_destroy(&dst);
                exit(-1);
