...
int src_h = 1080, src_w = 1920, dst_w = 128, dst_h = 1280, dev_id = 0;
...
static void * dwa_affine(void* arg) {
    bm_status_t ret;
    ...
    printf("idx:%d, bmcv_dwa_affine:loop %d cycles, time_max = %llu, time_avg = %llu, fps %llu, %lluM pps\n",
...
static void print_help(char **argv){
    printf("please follow this order:\n \
        %s src_w src_h dst_w dst_h fmt src_name dst_name u32RegionNum stDestSize.u32Width stDestSize.u32Height thread_num loop_num md5\n \
...
int main(int argc, char **argv) {
    ...
    if (argc >= 11) {
        ...
        src_name = argv[6];
        dst_name = argv[7];
        affine_attr.u32RegionNum = atoi(argv[8]);
        affine_attr.stDestSize.u32Width = atoi(argv[9]);
        affine_attr.stDestSize.u32Height = atoi(argv[10]);
    ...
    for (int i = 0; i < test_threads_num; i++) {
        ...
        if (pthread_create(
                &pid[i], NULL, dwa_affine, (void *)(ctx + i))) {
            free(pid);
