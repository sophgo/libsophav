...
static void fill_image(bm_image* src, uint8_t** raw_image, int n, int h, int w, int format) {
    float uv_format_factor = 0.5;
    ...
    if (*raw_image == NULL) {
        *raw_image = (uint8_t*)malloc(n * h * w * 3);
        for(int j = 0; j < n; j++){
            // Init Y
            for(int i = 0; i < h; i++) {
    ...
    uint8_t* input_ptr = malloc(sizeof(uint8_t) * n * n_stride);
    ...
    for (int i = 0; i < n; i++) {
        uint8_t* image_buf[3] = {(uint8_t*)malloc(h * stride[0]),
                            (uint8_t*)malloc(h * stride[1]),
                            (uint8_t*)malloc(h * stride[2])};
        switch(format)
        {
            case FORMAT_YUV420P:
...
#endif
{
    ...
    for (int loop = 0; loop < loop_num; loop++) {
        ...
        printf("input image width is: %d, height is: %d, stride is: %d\n", image_w, image_h, stride[0]);
        ...
        if (true) {  // create_dst_image
            ...
            int dst_format = (format == FORMAT_NV12 || format == FORMAT_NV21 || format == FORMAT_YUV420P) ?
                              ...
                              ((format == FORMAT_NV16 || format == FORMAT_NV61 || format == FORMAT_YUV422P) ?
            ...
            stride[0] = image_w + rand() % 10 * 4;
            printf("output image width is: %d, height is: %d, stride is: %d\n", image_w, image_h, stride[0]);
        ...
        for (int i = 0;i < image_n; i++) {
            void* out_ptr[3] = {(void*)((uint8_t*)output_ptr + i * byte_size),
                                (void*)((uint8_t*)output_ptr + i * byte_size + image_byte_size[0]),
                                (void*)((uint8_t*)output_ptr + i * byte_size + image_byte_size[0] + image_byte_size[1])};
        ...
        int cmp_res = image_cmp_u8_with_stride((uint8_t*)raw_image,
                                               (uint8_t*)output_ptr,
                                               format,
                                               dst->image_format,
...
void test_jpeg_dec(char* file) {
    ...
    size_t size = 0;
    uint8_t* jpeg_data = (uint8_t*)malloc(IMAGE_H * IMAGE_W * 3);
    uint8_t* output_ptr = (uint8_t*)malloc(IMAGE_H * IMAGE_W * 3);
...
#endif
    int byte_size = image_byte_size[0] + image_byte_size[1] + image_byte_size[2];
    void* out_ptr[3] = {(void*)((uint8_t*)output_ptr),
                        (void*)((uint8_t*)output_ptr + image_byte_size[0]),
                        (void*)((uint8_t*)output_ptr + image_byte_size[0] + image_byte_size[1])};
...
void test_jpeg_enc(char* in_file)
{
    ...
    int ret = 0;
    uint8_t* raw_image = (uint8_t*)malloc(IMAGE_H * IMAGE_W * 3);
    size_t in_size = 0;
    ...
    void* jpeg_data = NULL;
    size_t out_size = 0;
    ret = bmcv_image_jpeg_enc(handle, 1, &src, &jpeg_data, &out_size, 95);
    if (ret != BM_SUCCESS) exit(-1);
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 5) {
        printf("command input error, please follow this order:\n"
               "\ttest_cv_jpeg loop_num multi_thread_num dev_id random_seed\n");
        exit(-1);
    }
