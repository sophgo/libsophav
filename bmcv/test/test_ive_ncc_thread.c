...
static void *ive_ncc(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(golden_sample_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive ncc] _cmp failed, cmp = %d \n", cmp);
                free(ncc_result);
                exit(-1);
            }
            printf("[bmcv ive ncc] _cmp successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp((unsigned char*)ncc_result, (unsigned char*)golden_sample_name, data_len);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive ncc] cmp_md5 failed, cmp = %d \n", cmp);
                free(ncc_result);
                exit(-1);
            }
        }
...
int main(int argc, char **argv) {
    ...
    if (argc == 2)
        ...
        %s width height src1_name src2_name dst_name golden_sample_name dev_id thread_num loop_num bWrite\n \
        %s 352 288 ive_data/00_352x288_y.yuv ive_data/01_352x288_y.yuv dst_bin ive_data/result/sample_NCC_Mem.bin 0 1 5 0\n", argv[0], argv[0]);
        exit(-1);
    }
