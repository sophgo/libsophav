...
static void *ive_stcandicorner(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive stCandiCorner] cmp_u8 failed, cmp = %d \n", cmp);
                bm_image_destroy(&src);
                bm_image_destroy(&dst);
                bm_free_device(handle, stCandiCorner_attr.st_mem);
                free(ive_res);
            ...
            printf("[bmcv ive stCandiCorner] cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp((unsigned char*)ive_res, (unsigned char*)ref_name, width * height * sizeof(unsigned char));
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive stCandiCorner] cmp_md5 failed, cmp = %d \n", cmp);
                bm_image_destroy(&src);
                bm_image_destroy(&dst);
                bm_free_device(handle, stCandiCorner_attr.st_mem);
                free(ive_res);
