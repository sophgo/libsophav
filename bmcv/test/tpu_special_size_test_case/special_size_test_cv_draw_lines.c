                        ...
                        unsigned char color[3], int thickness, long int* cpu_time)
{
    ...
    printf("Draw_line CPU using time: %ld(us)\n", TIME_COST_US(t1, t2));
...
static int draw_line_tpu(bm_handle_t handle, unsigned char* input, int height, int width, int line_num,
                        ...
                        unsigned char color[3], int thickness, long int* tpu_time)
{
    ...
    gettimeofday(&t2, NULL);
    printf("Draw_line TPU using time: %ld(us)\n", TIME_COST_US(t1, t2));
...
static int test_draw_line_case(bm_handle_t handle, char* input, char* output, int height,
                            int width, int format, long int* cpu_time, long int* tpu_time)
{
    ...
    ret = draw_line_cpu(data_cpu, height, width, line_num, format, start, end, color, thickness, cpu_time);
    if (ret != 0) {
        printf("draw_line_cpu failed!\n");
        goto exit;
    }

    ret = draw_line_tpu(handle, data_tpu, height, width, line_num, format, start, end, color, thickness, tpu_time);
    if (ret != 0) {
...
//             exit(-1);
//         }
//         printf("------Test Draw_line Successed!------\n");
//     }
//     return (void*)0;
// }
...
int main(int argc, char* args[])
{
    printf("log will be saved in special_size_test_cv_dl.txt\n");
    FILE *original_stdout = stdout;
    FILE *file = freopen("special_size_test_cv_dl.txt", "w", stdout);
    if (file == NULL) {
    ...
    for (i = 0; i < 10; ++i) {
        ...
        for(loop = 0; loop < 10; loop++) {
            ret = test_draw_line_case(handle, NULL, NULL, height, width, format, &cputime, &tputime);
            if (ret) {
                printf("------TEST DRAW_LINE FAILED------\n");
                return -1;
            }
