...
static int tpu_gemm_ext(bm_handle_t handle, float* A, float* B, float* C, float* tpu_res,
                        ...
                        int M, int N, int K, float alpha, float beta, bool is_A_trans, bool is_B_trans, long int* tpu_time)
{
...
static int tpu_gemm(bm_handle_t handle, bool if_A_trans, bool if_B_trans, int M, int N, int K, float alpha,
                    float* src_A, int lda, float* src_B, int ldb, float beta, float* src_C, long int* tpu_time)
{
    struct timeval t1, t2;
    bm_status_t ret = BM_SUCCESS;
...
static int test_gemm(bm_handle_t handle, int M, int N, int K, float alpha,
                    float beta, bool is_A_trans, bool is_B_trans, long int* cpu_time, long int* tpu_time)
{
    ...
    *cpu_time += TIME_COST_US(t1, t2);
    ...
    ret = tpu_gemm(handle, is_A_trans, is_B_trans, M, N, K, alpha, A, lda, B, ldb, beta, tpu_C, tpu_time);
    if (ret) {
...
static int test_gemm_ext(bm_handle_t handle, int M, int N, int K, float alpha,
                        float beta, bool is_A_trans, bool is_B_trans, long int* cpu_time, long int* tpu_time_1, long int* tpu_time_2, long int* tpu_time_3)
{
    // printf("%s: M = %d, N = %d, K = %d, alpha = %f, beta = %f, is_A_trans = %d, is_B_trans = %d\n",
    //        __func__, M, N, K, alpha, beta, is_A_trans, is_B_trans);
    ...
    *cpu_time += TIME_COST_US(t1, t2);
    ...
    memset(tpu_C, 0.f, sizeof(float) * M * N);
    ret = tpu_gemm_ext(handle, A, B, C, tpu_C, in_dtype, out_dtype, M, N, K, alpha, beta, is_A_trans, is_B_trans, tpu_time_1);
    if (ret) {
    ...
    if (!(is_A_trans && M > 64)) {
        ...
        memset(tpu_C, 0.f, sizeof(float) * M * N);
        ret = tpu_gemm_ext(handle, A, B, C, tpu_C, in_dtype, out_dtype, M, N, K, alpha, beta, is_A_trans, is_B_trans, tpu_time_2);
        if (ret) {
            printf("%s: tpu_gemm_ext failed!\n", __func__);
            goto exit;
        }
        ...
        out_dtype = DATA_TYPE_EXT_FLOAT32;
        memset(tpu_C, 0.f, sizeof(float) * M * N);
        ret = tpu_gemm_ext(handle, A, B, C, tpu_C, in_dtype, out_dtype, M, N, K, alpha, beta, is_A_trans, is_B_trans, tpu_time_3);
        if (ret) {
            printf("%s: tpu_gemm_ext failed!\n", __func__);
            goto exit;
        }
...
//         }
//     }
//     printf("------Test Gemm_all Successed!------\n");
//     return (void*)0;
// }
...
int main(int argc, char* args[])
{
    printf("log will be saved in special_size_test_cv_gemm.txt\n");
    FILE *original_stdout = stdout;
    FILE *file = freopen("special_size_test_cv_gemm.txt", "w", stdout);
    if (file == NULL) {
    ...
    srand(seed);
    int loop = 0;
    int M_num[8] = {1, 16, 64, 100, 256, 400, 512, 800};
    int N_num[8] = {1, 16, 64, 100, 256, 400, 512, 800};
    int K_num[8] = {1, 16, 64, 100, 256, 400, 512, 800};
    bool if_A_trans_num[3] = {0, 0, 1};
    bool if_B_trans_num[3] = {0, 1, 1};
    int M, N, K;
    bool if_A_trans, if_B_trans;
    float alpha = 0.5f;
    float beta = -0.5f;
    bm_handle_t handle;
    int i, j;
    ...
    for (i = 0; i < 3; ++i) {
        if_A_trans = if_A_trans_num[i];
        if_B_trans = if_B_trans_num[i];
        for (j = 0; j < 8; ++j) {
            M = M_num[j];
            N = N_num[j];
            K = K_num[j];
            printf("the M = %d, the N = %d, the K = %d, if_A_trans = %d, if_B_trans = %d\n", \
                    M, N, K, if_A_trans, if_B_trans);
            long int cputime = 0;
            long int tputime1 = 0;
            long int tputime2 = 0;
            long int tputime3 = 0;
            long int cputime_gemm = 0;
            long int tputime_gemm = 0;
            for(loop = 0; loop < 10; loop++) {
                ret = test_gemm_ext(handle, M, N, K, alpha, beta, if_A_trans, if_B_trans, &cputime, &tputime1, &tputime2, &tputime3);
                if (ret) {
                    printf("------TEST CV_GEMM_EXT FAILED------\n");
                    return -1;
                }
                ret = test_gemm(handle, M, N, K, alpha, beta, if_A_trans, if_B_trans, &cputime_gemm, &tputime_gemm);
                if (ret) {
                    printf("------TEST CV_GEMM FAILED------\n");
                    return -1;
                }
            }
            printf("-----------average GEMM_EXT CPU time = %ldus----------\n", cputime / 10);
            printf("-----------average GEMM_EXT FLOAT32 -> FLOAT32 TPU time = %ldus----------\n", tputime1 / 10);
            printf("-----------average GEMM_EXT FP16 -> FP16 TPU time = %ldus----------\n", tputime2 / 10);
            printf("-----------average GEMM_EXT FP16 -> FLOAT32 TPU time = %ldus----------\n", tputime3 / 10);
            printf("\n");
            printf("-----------average GEMM CPU time = %ldus----------\n", cputime_gemm / 10);
            printf("-----------average GEMM TPU time = %ldus----------\n", tputime_gemm / 10);
        }
    }
    bm_dev_free(handle);
    fclose(stdout);
