...
static int tpu_hist_balance(uint8_t* input_host, uint8_t* output_tpu, int height, int width, bm_handle_t handle, long int* tpu_time)
{
...
static int test_hist_balance(bm_handle_t handle, int height, int width, long int* cpu_time, long int* tpu_time)
{
    ...
    *cpu_time += TIME_COST_US(t1, t2);
    ...
    ret = tpu_hist_balance(inputHost, output_tpu, height, width, handle, tpu_time);
    if (ret) {
...
int main(int argc, char* args[])
{
    printf("log will be saved in special_size_test_cv_hist_balance.txt\n");
    FILE *original_stdout = stdout;
    FILE *file = freopen("special_size_test_cv_hist_balance.txt", "w", stdout);
    if (file == NULL) {
    ...
    srand(seed);
    int loop = 0;
    int height_num[10] = {1, 256, 512, 1024, 720, 1080, 1440, 2160, 4096, 8192};
    int width_num[10] = {1, 256, 512, 1024, 1280, 1920, 2560, 3840, 4096, 8192};
    int height, width;
    bm_handle_t handle;
    int i;
    ...
    for (i = 0; i < 10; ++i) {
        height = height_num[i];
        width = width_num[i];
        printf("the input_hieght = %d, the input_width = %d\n", height, width);
        long int cputime = 0;
        long int tputime = 0;
        for(loop = 0; loop < 10; loop++) {
            ret = test_hist_balance(handle, height, width, &cputime, &tputime);
            if (ret) {
                printf("------TEST CV_HIST_BALANCE FAILED------\n");
                return -1;
            }
        }
