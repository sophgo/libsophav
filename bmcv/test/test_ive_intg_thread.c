...
static void *ive_intg(void* arg) {
    ...
    if (ctx.i == 0) {
        if (data_type == 0) {
            ...
            if (strchr(golden_sample_name, '.') != NULL) {
                int cmp = cmp_u32(golden_sample_name, dst_intg_u32, width * height);
                if (cmp != 0) {
                    printf("[bmcv ive intg] cmp_u32 failed, cmp = %d \n", cmp);
                    free(dst_intg_u32);
                    exit(-1);
                }
                printf("[bmcv ive intg] cmp_u32 successful, cmp = %d \n", cmp);
            }
            else {
                int cmp = md5_cmp((unsigned char*)dst_intg_u32, (unsigned char*)golden_sample_name, width * height * data_size);
                if (cmp != BM_SUCCESS) {
                    printf("[bmcv ive intg] cmp_md5 failed, cmp = %d \n", cmp);
                    free(dst_intg_u32);
                    exit(-1);
                }
            }
            ...
            if (strchr(golden_sample_name, '.') != NULL) {
                ...
                if (cmp != 0) {
                    printf("[bmcv ive intg] cmp_u64 failed, cmp = %d \n", cmp);
                    free(dst_intg_u64);
                    exit(-1);
                }
                printf("[bmcv ive intg] cmp_u64 successful, cmp = %d \n", cmp);
            }
            else {
                int cmp = md5_cmp((unsigned char*)dst_intg_u64, (unsigned char*)golden_sample_name,  width * height * data_size);
                if (cmp != BM_SUCCESS) {
                    printf("[bmcv ive intg] cmp_md5 failed, cmp = %d \n", cmp);
                    free(dst_intg_u64);
                    exit(-1);
            ...
            if (bWrite) {
                FILE *intg_result_fp = fopen(dst_name, "wb");
                fwrite((void *)dst_intg_u64, data_size, width * height, intg_result_fp);
                fclose(intg_result_fp);
            }
            free(dst_intg_u64);
        }
...
int main(int argc, char **argv) {
    ...
    if (argc == 2)
        ...
        %s width height integ_mode data_type src_name dst_name golden_sample_name dev_id thread_num loop_num bWrite\n \
        %s 352 288 2 1 ive_data/00_352x288_y.yuv integ_sqsum.bin ive_data/result/sample_Integ_Sqsum.yuv 0 1 1 0\n", argv[0], argv[0]);
        exit(-1);
    }
