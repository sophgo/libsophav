...
static void *ive_add(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            ...
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive add] cmp_md5 failed, cmp = %d \n", cmp);
                bm_image_destroy(&src1);
                bm_image_destroy(&src2);
                bm_image_destroy(&dst);
                free(ive_add_res);
...
int main(int argc, char **argv) {
    ...
    if ((argc > 3 && argc < 9) || (argc == 1)) {
        ...
        %s 352 288 14 14 19584 45952 ive_data/00_352x288_y.yuv ive_data/01_352x288_y.yuv ive_data/result/sample_Add.yuv 0 1 1 0\n", argv[0], argv[0]);
        exit(-1);
    }
