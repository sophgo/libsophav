...
static void *ive_gradfg(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive gradFg] cmp failed, cmp = %d \n", cmp);
                free(gradFg_res);
                bm_image_destroy(&stBgdiffFg);
                bm_image_destroy(&stCurGrad);
                bm_image_destroy(&stBgGrad);
            ...
            printf("[bmcv ive gradFg] cmp successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(gradFg_res, (unsigned char*)ref_name, width * height * sizeof(unsigned char));
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive gradFg] cmp_md5 failed, cmp = %d \n", cmp);
                free(gradFg_res);
                bm_image_destroy(&stBgdiffFg);
                bm_image_destroy(&stCurGrad);
                bm_image_destroy(&stBgGrad);
