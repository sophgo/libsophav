...
bm_status_t init_match_bgmodel(bm_handle_t handle, bm_image* pstCurImg,
    ...
    bm_ive_image_calc_stride(handle, height, width * sizeof(bmcv_ive_bg_model_pix), fmt, DATA_TYPE_EXT_1N_BYTE, bgModelStride);
    ...
    ret = bm_image_create(handle, height, width * sizeof(bmcv_ive_bg_model_pix), fmt, DATA_TYPE_EXT_1N_BYTE, pstBgModel, bgModelStride);
    if (ret != BM_SUCCESS) {
    ...
    for (i = 0; i < loop_time; i++) {
        ...
        for (frmCnt = 0; frmCnt < frameNumMax; frmCnt++) {
            ...
            for (int j = 0; j < height; j++)
                memcpy(src_data + j * width, input_data + (frmCnt * width * height + j * width), width);
            ...
            match_time_single = (unsigned int)(timediff.tv_sec * 1000000 + timediff.tv_usec);
#endif
            if (match_time_single > match_time_max)
                match_time_max = match_time_single;
            if (match_time_single < match_time_min)
                match_time_min = match_time_single;
            ...
            if (updCnt == 0 || frameNum >= preUpdTime + frmUpdPeriod) {
                ...
                update_time_single = (unsigned int)(timediff.tv_sec * 1000000 + timediff.tv_usec);
#endif
                if (update_time_single > update_time_max)
                    update_time_max = update_time_single;
                if (update_time_single < update_time_min)
                    update_time_min = update_time_single;
        ...
        if (strchr(ref_name, '.') != NULL) {
            int cmp = cmp_u8(ref_name, bgmodel_res, width * sizeof(bmcv_ive_bg_model_pix) * height);
            if (cmp != 0) {
                printf("[bmcv ive bgmodel] cmp_u8 failed, cmp = %d \n", cmp);
                exit(-1);
            }
            printf("[bmcv ive bgmodel] cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(bgmodel_res, (unsigned char*)ref_name, width * sizeof(bmcv_ive_bg_model_pix) * height);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive bgmodel] cmp_md5 failed, cmp = %d \n", cmp);
                exit(-1);
            }
        }
    }
...
int main(int argc, char **argv) {
    if (argc >= 5) {
        ...
        if (argc > 10) frameNumMax = atoi(argv[10]);
    }
    if (argc == 2)
        ...
        %s width height src_name ref_name dev_id thread_num loop_num bWrite bgModel_name frameNumMax\n \
