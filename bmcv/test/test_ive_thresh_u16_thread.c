...
static void * ive_thresh_u16(void* arg) {
    ...
    if(ctx.i == 0){
        ...
        if(cmp != 0){
            printf("[bmcv ive thresh_u16] cmp failed, cmp = %d \n", cmp);
        } else {
            printf("[bmcv ive thresh_u16] cmp successful, cmp = %d \n", cmp);
        }
    ...
    char algorithm_str[100] = "ive_thresh_u16";
    ...
    printf("idx:%d, bm_ive_thresh_u16: loop %d cycles, time_max = %llu, time_avg = %llu, fps %llu, %lluM pps\n",
...
int main(int argc, char **argv) {
    ...
    if (test_loop_times > 15000 || test_loop_times < 1) {
        printf("[TEST ive thresh_u16] loop times should be 1~15000\n");
        exit(-1);
    }
    if (test_threads_num > MAX_THREAD_NUM || test_threads_num < 1) {
        printf("[TEST ive thresh_u16] thread nums should be 1~%d\n", MAX_THREAD_NUM);
        exit(-1);
    }
