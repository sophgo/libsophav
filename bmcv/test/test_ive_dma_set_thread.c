...
char *src_name = "./ive_data/00_352x288_y.yuv", *ref_name = "./ive_data/00_352x288_y.yuv", *dst_name = "sample_DMA_set_Direct.bin";
...
static void *ive_dma_set(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            ...
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive dma_set] cmp failed, cmp = %d \n", cmp);
                exit(-1);
            }
            printf("[bmcv ive dma_set] cmp successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(ive_res, (unsigned char*)ref_name, width * height * sizeof(unsigned char));
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive dma_set] cmp_md5 failed, cmp = %d \n", cmp);
                exit(-1);
            }
            printf("[bmcv ive dma_set] cmp_md5 successful, cmp = %d \n", cmp);
        }
        free(ive_res);
    }
    ...
    printf("idx:%d, %d*%d->%d*%d, %s, %s\n", ctx.i, width, height, width, height, src_fmt_str, algorithm_str);
...
int main(int argc, char **argv) {
    ...
    if ((argc > 3 && argc < 7) || (argc == 1)) {
        ...
        %s width height dma_set_mode val src_fmt src_name ref_name dev_id thread_num loop_num bWrite dst_name\n \
