...
static void *ive_gmm(void* arg) {
    ...
    if (src_fmt == FORMAT_RGB_PACKED)
        data_size = 3;
    int model_len = width * height * data_size * gmmAttr.u8_model_num * 8;
    ...
    fread((void *)inputData, sizeof(unsigned char), width * height * data_size * u32FrameNumMax * sizeof(unsigned char), input_fp);
    fclose(input_fp);
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(goldenFg_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive gmm] dstFg cmp_u8 failed, cmp = %d \n", cmp);
                free(inputData);
                free(ive_fg_res);
                free(srcData);
                free(ive_bg_res);
            ...
            printf("[bmcv ive gmm] dstFg cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(ive_fg_res, (unsigned char*)goldenFg_name, width * height);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive gmm] dstFg cmp_md5 failed, cmp = %d \n", cmp);
                free(inputData);
                free(ive_fg_res);
                free(srcData);
                free(ive_bg_res);
            ...
            printf("[bmcv ive gmm] dstFg cmp_md5 successful, cmp = %d \n", cmp);
        }
        if (strchr(goldenBg_name, '.') != NULL) {
            int cmp = cmp_u8(goldenBg_name, ive_bg_res, width * height);
            if (cmp != 0) {
                printf("[bmcv ive gmm] dstBg cmp_u8 failed, cmp = %d \n", cmp);
                free(inputData);
                free(ive_fg_res);
                free(srcData);
                free(ive_bg_res);
            ...
            printf("[bmcv ive gmm] dstBg cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(ive_bg_res, (unsigned char*)goldenBg_name, width * height);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive gmm] dstBg cmp_md5 failed, cmp = %d \n", cmp);
                free(inputData);
                free(ive_fg_res);
                free(srcData);
                free(ive_bg_res);
            ...
            printf("[bmcv ive gmm] dstBg cmp_md5 successful, cmp = %d \n", cmp);
        }
        if (bWrite) {
            FILE *fg_fp = fopen(dstFg_name, "wb");
            fwrite((void *)ive_fg_res, 1, width * height * u32FrameNumMax * sizeof(unsigned char), fg_fp);
            fclose(fg_fp);
            FILE *bg_fp = fopen(dstBg_name, "wb");
            fwrite((void *)ive_bg_res, 1, width * height * u32FrameNumMax * sizeof(unsigned char), bg_fp);
            fclose(bg_fp);
        }
    }
    free(inputData);
...
int main(int argc, char **argv) {
    ...
    if (argc == 2)
        ...
        %s width height src_fmt src_name input_name goldenFg_name goldenBg_name dev_id thread_num loop_num bWrite dstFg_name dstBg_name\n \
