...
static void *ive_hist(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(golden_sample_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive hist] cmp failed, cmp = %d \n", cmp);
                free(dst_hist_result);
                exit(-1);
            }
            printf("[bmcv ive hist] cmp successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(dst_hist_result, (unsigned char*)golden_sample_name, dst.size);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive hist] cmp_md5 failed, cmp = %d \n", cmp);
                free(dst_hist_result);
                exit(-1);
            }
        }
...
int main(int argc, char **argv) {
    ...
    if (argc == 2)
        ...
        %s width height src_name dst_name golden_sample_name dev_id thread_num loop_num bWrite\n \
        %s 352 288 ive_data/00_352x288_y.yuv ./hist_res.bin ive_data/result/sample_Hist.bin 0 1 5 0\n", argv[0], argv[0]);
        exit(-1);
    }
