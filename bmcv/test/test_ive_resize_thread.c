...
int src_width = 352, src_height = 288;
...
static void *ive_resize(void* arg) {
    ...
    for (int img_idx = 0; img_idx < IMG_NUM; img_idx++) {
        bm_ive_image_calc_stride(handle, src_height, src_width,
                                 format[img_idx], DATA_TYPE_EXT_1N_BYTE, src_stride[img_idx]);
        bm_image_create(handle, src_height, src_width, format[img_idx],
    ...
    if (ctx.i == 0) {
        for (int j = 0; j < IMG_NUM; j++) {
            ...
            if (strchr(ref_name[j], '.') != NULL) {
                int cmp = cmp_u8(ref_name[j], output_ptr, byte_size);
                if (cmp != 0) {
                    printf("[bmcv ive resize] [img_num %d] cmp_u8 failed, cmp = %d \n", j, cmp);
                    for (int idx = 0; idx < IMG_NUM; idx++) {
                        bm_image_destroy(&src[idx]);
                        bm_image_destroy(&dst[idx]);
                    }
                ...
                printf("[bmcv ive resize] [img_num %d] cmp_u8 successful, cmp = %d \n", j, cmp);
            }
            else {
                int cmp = md5_cmp((unsigned char*)output_ptr, (unsigned char*)ref_name[j], byte_size);
                if (cmp != BM_SUCCESS) {
                    printf("[bmcv ive resize] [img_num %d] cmp_md5 failed, cmp = %d \n", j, cmp);
                    for (int idx = 0; idx < IMG_NUM; idx++) {
                        bm_image_destroy(&src[idx]);
                        bm_image_destroy(&dst[idx]);
                    }
    ...
    for (int img_idx = 0; img_idx < IMG_NUM; img_idx++) {
        ...
        printf("bmcv_ive_resize: fmt is %s, resize_mode is %s, size is %d x %d -->> %d x %d\n",
               fmt_str[img_idx], enMode_str, src_width, src_height, dst_width[img_idx], dst_height[img_idx]);
    }
...
int main(int argc, char **argv) {
    if (argc >= 5) {
        ...
        if (argc > 9) src_width  = atoi(argv[9]);
        if (argc > 10) src_height  = atoi(argv[10]);
        if (argc > 11) src_name[0]  = argv[11];
        if (argc > 12) src_name[1]  = argv[12];
        if (argc > 13) src_name[2]  = argv[13];
        if (argc > 14) dst_name[0]  = argv[14];
        if (argc > 15) dst_name[1]  = argv[15];
        if (argc > 16) dst_name[2]  = argv[16];
        if (argc > 17) dst_width[0]  = atoi(argv[17]);
        if (argc > 18) dst_height[0]  = atoi(argv[18]);
        if (argc > 19) dst_width[1]  = atoi(argv[19]);
        if (argc > 20) dst_height[1]  = atoi(argv[20]);
        if (argc > 21) dst_width[2]  = atoi(argv[21]);
        if (argc > 22) dst_height[2]  = atoi(argv[22]);
    }
    if (argc == 2)
        ...
        %s resize_mode ref_name1 ref_name2 ref_name3 bWrite dev_id thread_num loop_num src_width src_height src_name1 src_name2 src_name3 dst_name1 dst_name2 dst_name3 dst_width1 dst_height1 dst_width2 dst_height2 dst_width3 dst_height3\n \
