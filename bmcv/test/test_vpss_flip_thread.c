...
char *md5 = "37890f4ab5d6458b57824de9e24ba94f";
...
static void * flip(void* arg) {
    ...
    printf("idx:%d, bmcv_image_flip:loop %d cycles, time_max = %llu, time_avg = %llu, fps %llu, %lluM pps\n",
...
static void print_help(char **argv){
    printf("please follow this order:\n \
        %s src_w src_h src_fmt src_name flip_mode dst_name dev_id thread_num loop_num\n \
...
int main(int argc, char **argv) {
    ...
    if (argc >= 8) {
        ...
        src_name = argv[4];
        flip_mode = (bmcv_flip_mode)atoi(argv[5]);
        dst_w = src_w;
        dst_h = src_h;
        dst_fmt = src_fmt;
        dst_name = argv[6];
