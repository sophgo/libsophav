...
static void *ive_sub(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive sub] cmp_u8 failed, cmp = %d \n", cmp);
                exit(-1);
            }
            printf("[bmcv ive sub] cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp((unsigned char*)ive_sub_res, (unsigned char*)ref_name, width * height * sizeof(unsigned char));
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive sub] cmp_md5 failed, cmp = %d \n", cmp);
                exit(-1);
            }
        }
        if (bWrite) {
...
int main(int argc, char **argv) {
    ...
    if ((argc > 3 && argc < 8) || (argc == 1)) {
        ...
        %s 352 288 1 14 14 ive_data/00_352x288_y.yuv ive_data/01_352x288_y.yuv ive_data/result/sample_Sub_Shift.yuv 0 1 5 0\n", argv[0], argv[0]);
        exit(-1);
    }
