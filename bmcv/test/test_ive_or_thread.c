...
char *ref_name = "./ive_data/result/sample_Or.yuv";
...
static void *ive_or(void* arg) {
    bm_status_t ret;
    ive_or_ctx ctx = *(ive_or_ctx*)arg;
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive or] cmp_u8 failed, cmp = %d \n", cmp);
                exit(-1);
            }
            printf("[bmcv ive or] cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp((unsigned char*)ive_res, (unsigned char*)ref_name, width * height);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive or] cmp_md5 failed, cmp = %d \n", cmp);
                exit(-1);
            }
        }
        if (bWrite) {
    ...
    printf("idx:%d, bm_ive_or: loop %d cycles, time_max = %llu, time_avg = %llu, fps %llu, %lluM pps\n",
...
int main(int argc, char **argv) {
    ...
    if (test_loop_times > 15000 || test_loop_times < 1) {
        printf("[TEST ive or] loop times should be 1~15000\n");
        exit(-1);
    }
    if (test_threads_num > MAX_THREAD_NUM || test_threads_num < 1) {
        printf("[TEST ive or] thread nums should be 1~%d\n", MAX_THREAD_NUM);
        exit(-1);
    }
