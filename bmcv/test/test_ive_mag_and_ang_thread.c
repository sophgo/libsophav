...
static void *ive_magAndAng(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(golden_Magsample_name, '.') != NULL) {
            int cmp = cmp_u8(golden_Magsample_name, (unsigned char*)magOut_res, width * height * sizeof(unsigned short));
            if (cmp != 0) {
                printf("[bmcv ive magAndAng] mag cmp_u8 failed, cmp = %d \n", cmp);
                free(magOut_res);
                free(angOut_res);
                exit(-1);
            }
            printf("[bmcv ive magAndAng] mag cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp((unsigned char*)magOut_res, (unsigned char*)golden_Magsample_name, width * height * sizeof(unsigned short));
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive magAndAng] mag cmp_md5 failed, cmp = %d \n", cmp);
                free(magOut_res);
                free(angOut_res);
                exit(-1);
            }
        ...
        if (enMode == BM_IVE_MAG_AND_ANG_OUT_ALL) {
            ...
            if (strchr(golden_Angsample_name, '.') != NULL) {
                int cmp = cmp_u8(golden_Angsample_name, angOut_res, width * height * sizeof(unsigned char));
                if (cmp != 0) {
                    printf("[bmcv ive magAndAng] ang cmp_u8 failed, cmp = %d \n", cmp);
                    free(magOut_res);
                    free(angOut_res);
                    exit(-1);
                }
                printf("[bmcv ive magAndAng] ang cmp_u8 successful, cmp = %d \n", cmp);
            }
            else {
                int cmp = md5_cmp((unsigned char*)angOut_res, (unsigned char*)golden_Angsample_name, width * height * sizeof(unsigned char));
                if (cmp != BM_SUCCESS) {
                    printf("[bmcv ive magAndAng] ang cmp_md5 failed, cmp = %d \n", cmp);
                    free(magOut_res);
                    free(angOut_res);
                    exit(-1);
                }
        ...
        if (bWrite) {
            ...
            fwrite((void *)magOut_res, sizeof(unsigned short), width * height, mag_fp);
            fclose(mag_fp);
            if (enMode == BM_IVE_MAG_AND_ANG_OUT_ALL) {
                FILE *ang_fp = fopen(dst_angName, "wb");
                fwrite((void *)angOut_res, sizeof(unsigned char), width * height, ang_fp);
                fclose(ang_fp);
            }
        }
        free(magOut_res);
