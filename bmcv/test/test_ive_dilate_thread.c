...
static void *ive_dilate(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(golden_sample_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive dilate] cmp failed, cmp = %d \n", cmp);
                free(ive_dilate_res);
                exit(-1);
            }
            printf("[bmcv ive dilate] cmp successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(ive_dilate_res, (unsigned char*)golden_sample_name, width * height);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive dilate] cmp_md5 failed, cmp = %d \n", cmp);
                free(ive_dilate_res);
                exit(-1);
            }
        }
