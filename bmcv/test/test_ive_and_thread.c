...
char *dst_name = "sample_And.yuv";
char *ref_name = "./ive_data/result/sample_And.yuv";
...
static void *ive_and(void* arg) {
    bm_status_t ret;
    ive_and_ctx ctx = *(ive_and_ctx*)arg;
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            int cmp = cmp_u8(ref_name, ive_and_res, width * height * sizeof(unsigned char));
            if (cmp != 0) {
                printf("[bmcv ive and] cmp failed, cmp = %d \n", cmp);
                bm_image_destroy(&src1);
                bm_image_destroy(&src2);
                bm_image_destroy(&dst);
                free(ive_and_res);
            ...
            printf("[bmcv ive and] cmp successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(ive_and_res, (unsigned char*)ref_name, width * height * sizeof(unsigned char));
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive and] cmp_md5 failed, cmp = %d \n", cmp);
                bm_image_destroy(&src1);
                bm_image_destroy(&src2);
                bm_image_destroy(&dst);
                free(ive_and_res);
    ...
    bm_image_destroy(&src2);
    bm_image_destroy(&dst);
    char algorithm_str[100] = "ive_and";
    ...
    printf("idx:%d, bm_ive_and: loop %d cycles, time_max = %llu, time_avg = %llu, fps %llu, %lluM pps\n",
...
int main(int argc, char **argv) {
    ...
    if ((argc > 3 && argc < 7) || (argc == 1)) {
        ...
        %s 352 288 14 14 ive_data/00_352x288_y.yuv ive_data/01_352x288_y.yuv ive_data/result/sample_And.yuv 0 1 1 0\n", argv[0], argv[0]);
        exit(-1);
    }
    if (test_loop_times > 15000 || test_loop_times < 1) {
        printf("[TEST ive and] loop times should be 1~15000\n");
        exit(-1);
    }
    if (test_threads_num > MAX_THREAD_NUM || test_threads_num < 1) {
        printf("[TEST ive and] thread nums should be 1~%d\n", MAX_THREAD_NUM);
        exit(-1);
    }
    ...
    ive_and_ctx ctx[MAX_THREAD_NUM];
#ifdef __linux__
