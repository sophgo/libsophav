...
char *ref_res_name = "./dpu_data/205pU8Disp_ref_512x284.bin";
char *dpu_res_name = "./out/sgbm_res.bin";
...
int bWrite = 0;
int rand_mode = 0;
bmcv_dpu_sgbm_mode   sgbm_mode = DPU_SGBM_MUX2;
        ...
        for(int j = 0; j < width; j++){
            if(got[i * ALIGN(width, 16) + j] != exp[i * width + j]){
                printf("Compare failed, at idx = %d, dpu_output = 0x%x, cmodel_output = 0x%x \n",
                           i, got[i * ALIGN(width, 16) + j], exp[i * width + j]);
                return -1;
            }
        }
    }
...
static void * test_dpu_sgbm(void* arg)
{
    dpu_sgbm_ctx ctx = *(dpu_sgbm_ctx*)arg;
    ...
    (sgbm_mode == DPU_SGBM_MUX1) ? (output_dtype = DATA_TYPE_EXT_U16) :
    ...
    for (i = 0; i < loop_time; i++) {
        ...
        ret = bmcv_dpu_sgbm_disp(handle, &left_img, &right_img, &disp_img, &sgbmAttrs, sgbm_mode);
        ...
        if(ret != BM_SUCCESS){
            printf("bmcv_dpu_sgbm_disp failed. ret = %d\n", ret);
    ...
    if(ctx.i == 0){
        if(rand_mode){
            if(sgbm_mode == DPU_SGBM_MUX1){
                ...
                unsigned short *u16_dpu_res= (unsigned short*) malloc (img_size * dsize);
                ...
                ret = test_cmodel_sgbm_u16(left_img_cmodel, right_img_cmodel, u16_ref_res,
                                    left_img.width, left_img.height, sgbm_mode, &sgbmAttrs);
                ...
                if(cmp != 0){
                    printf("[bmcv dpu sgbm] idx %d, u16_dpu_res cmp failed \n", ctx.i);
                    free(u16_dpu_res);
                    free(u16_ref_res);
                    free(left_img_cmodel);
                    free(right_img_cmodel);
                ...
                printf("[bmcv dpu sgbm] idx %d, u16_dpu_res cmp successful\n", ctx.i);
                ...
                unsigned char* left_img_cmodel = (unsigned char*) malloc (img_height * img_width * dsize);
                unsigned char* right_img_cmodel = (unsigned char*) malloc (img_height * img_width * dsize);

                copy_img_data(leftImg, left_img_cmodel, img_width, img_height);
                copy_img_data(rightImg, right_img_cmodel, img_width, img_height);

                ret = test_cmodel_sgbm(left_img_cmodel, right_img_cmodel, u8_ref_res,
                                   left_img.width, left_img.height, sgbm_mode, &sgbmAttrs);
                ...
                if(cmp != 0){
                    printf("[bmcv dpu sgbm] idx %d, u8_dpu_res cmp failed \n", ctx.i);
                    free(u8_ref_res);
                    free(u8_dpu_res);
                    free(left_img_cmodel);
                    free(right_img_cmodel);
                ...
                printf("[bmcv dpu sgbm] idx %d, u8_dpu_res cmp successful\n", ctx.i);
...
static void print_help(char **argv){
    printf("please follow this order:\n \
        %s width height sgbm_mode left_name right_name ref_name disp_range dev_id test_threads_num test_loop_times bWrite dpu_res_name\n \
...
int main(int argc, char **argv)
{
    ...
    if (test_threads_num > 8 || test_threads_num < 1) {
        printf("[TEST dpu sgbm] thread nums should be 1~8\n");
        exit(-1);
    }
    ...
    if(rand_mode){
        ...
        sgbm_mode = rand() % 3 + DPU_SGBM_MUX0;
    }

    dpu_sgbm_ctx ctx[test_threads_num];
    #ifdef __linux__
        ...
        for (int i = 0; i < test_threads_num; i++) {
            ...
            if (pthread_create(
                    &pid[i], NULL, test_dpu_sgbm, (void *)(ctx + i))) {
                free(pid);
