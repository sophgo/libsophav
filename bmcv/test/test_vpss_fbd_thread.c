...
bm_image_format_ext src_fmt = FORMAT_COMPRESSED, dst_fmt = FORMAT_YUV420P;
char *src_name0 = "/opt/sophon/libsophon-current/bin/res/fbc/1920x1080_table_y.bin";
char *src_name1 = "/opt/sophon/libsophon-current/bin/res/fbc/1920x1080_data_y.bin";
char *src_name2 = "/opt/sophon/libsophon-current/bin/res/fbc/1920x1080_table_c.bin";
char *src_name3 = "/opt/sophon/libsophon-current/bin/res/fbc/1920x1080_data_c.bin";
char *dst_name = "dst.bin";
int size[4] = {69632, 2073600, 34816, 1036800};
bmcv_rect_t rect = {.start_x = 0, .start_y = 0, .crop_w = 1920, .crop_h = 1080};
bmcv_resize_algorithm algorithm = BMCV_INTER_LINEAR;
bm_handle_t handle = NULL;
char *md5 = "b94ccf6ac082ae258a1c00d56ec6bb0f";
bm_status_t case_status = BM_SUCCESS;

static void * fbd(void* arg) {
    bm_status_t ret;
    ...
    int in_size = size[0] + size[1] + size[2] + size[3];
    unsigned char *input_ptr = (unsigned char *)malloc(in_size);
    void* in_ptr[4] = {input_ptr, input_ptr + size[0], input_ptr + size[0] + size[1], input_ptr + size[0] + size[1] + size[2]};
    FILE *fp_src[4];
    fp_src[0] = fopen(src_name0, "rb");
    fp_src[1] = fopen(src_name1, "rb");
    fp_src[2] = fopen(src_name2, "rb");
    fp_src[3] = fopen(src_name3, "rb");
    for (int i = 0; i < 4; i++) {
        if (fread((void *)in_ptr[i], 1, size[i], fp_src[i]) < (unsigned int)size[i]){
    ...
    if(md5 == NULL)
        ...
        if(md5_cmp(output_ptr, (unsigned char*)md5, byte_size)!=0){
            ...
            printf("thread_idx(%d) cmp error\n", ctx.i);
            ret = BM_ERR_FAILURE;
        } else
            printf("thread_idx(%d) cmp success\n", ctx.i);
        free(output_ptr);
    }
    ...
    printf("idx:%d, bmcv_image_fbd:loop %d cycles, time_max = %llu, time_avg = %llu, fps %llu, %lluM pps\n",
...
static void print_help(char **argv){
    printf("please follow this order:\n \
        %s src_w src_h src_table_y_size src_name0 src_name1 src_name2 src_name3 start_x start_y crop_w crop_h dst_w dst_h dst_fmt dst_name dev_id thread_num loop_num md5\n \
...
int main(int argc, char **argv) {
    ...
    if (argc >= 17) {
        ...
        size[1] = src_w * src_h;
        size[2] = size[0] >> 1;
        size[3] = src_w * src_h >> 1;
        src_name0 = argv[4];
        src_name1 = argv[5];
        src_name2 = argv[6];
        src_name3 = argv[7];
        rect.start_x = atoi(argv[8]);
        rect.start_y = atoi(argv[9]);
        rect.crop_w = atoi(argv[10]);
        rect.crop_h = atoi(argv[11]);
        dst_w = atoi(argv[12]);
        dst_h = atoi(argv[13]);
        dst_fmt = (bm_image_format_ext)atoi(argv[14]);
        dst_name = argv[15];
        dev_id = atoi(argv[16]);
    }
    if (argc == 2){
