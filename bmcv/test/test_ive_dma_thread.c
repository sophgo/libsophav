...
static void *ive_dma(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            ...
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive dma] cmp failed, cmp = %d \n", cmp);
                exit(-1);
            }
            printf("[bmcv ive dma] cmp successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(ive_res, (unsigned char*)ref_name, width * height * sizeof(unsigned char));
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive dma] cmp_md5 failed, cmp = %d \n", cmp);
                exit(-1);
            }
            printf("[bmcv ive dma] cmp_md5 successful, cmp = %d \n", cmp);
        }
        free(ive_res);
    }
...
int main(int argc, char **argv) {
    ...
    if ((argc > 3 && argc < 10) || (argc == 1)) {
        ...
        %s width height dma_mode hor_seg_size elem_size ver_seg_rows src_fmt dst_fmt src_name ref_name dev_id thread_num loop_num bWrite dst_name\n \
