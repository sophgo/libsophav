...
static void *ive_csc(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_dst_name, '.') != NULL) {
            int cmp = cmp_u8(ref_dst_name, output_ptr, byte_size);
            if (cmp != 0) {
                printf("[bmcv ive csc] cmp_u8 failed, cmp = %d \n", cmp);
                bm_image_destroy(&src);
                bm_image_destroy(&dst);
                free(output_ptr);
                exit(-1);
            }
            printf("[bmcv ive csc] cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(output_ptr, (unsigned char*)ref_dst_name, byte_size);
            if (cmp != BM_SUCCESS) {
