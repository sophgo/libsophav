...
static void *ive_gmm2(void* arg) {
    ...
    int stride[4], factorStride[4], dst_stride[4];
    ...
    if (src_fmt == FORMAT_GRAY)
        data_size = 1;
    if (src_fmt == FORMAT_RGB_PACKED)
        data_size = 3;
    unsigned char *inputData = malloc(width * height * data_size * u32FrameNumMax * sizeof(unsigned char));
    FILE *input_fp = fopen(input_name, "rb");
    fread((void *)inputData, sizeof(unsigned char), width * height * data_size * u32FrameNumMax, input_fp);
    fclose(input_fp);
    unsigned char *srcData = malloc(width * height * data_size * sizeof(unsigned char));
    memset(srcData, 0, width * height * data_size * sizeof(unsigned char));
    ...
    int model_len = width * height * data_size * gmm2Attr.u8_model_num * 8;
    ...
    bm_ive_image_calc_stride(handle, height, width, FORMAT_GRAY, DATA_TYPE_EXT_U16, factorStride);
    ...
    bm_ive_image_calc_stride(handle, height, width, FORMAT_GRAY, DATA_TYPE_EXT_1N_BYTE, dst_stride);
    bm_image_create(handle, height, width, FORMAT_GRAY, DATA_TYPE_EXT_1N_BYTE, &dst_fg, dst_stride);
    ret = bm_image_alloc_dev_mem(dst_fg, BMCV_HEAP1_ID);
    if (ret != BM_SUCCESS) {
        printf("bm_image_alloc_dev_mem_src. ret = %d\n", ret);
        goto fail;
    }
    bm_image_create(handle, height, width, FORMAT_GRAY, DATA_TYPE_EXT_1N_BYTE, &dst_bg, dst_stride);
    ret = bm_image_alloc_dev_mem(dst_bg, BMCV_HEAP1_ID);
    if (ret != BM_SUCCESS) {
        printf("bm_image_alloc_dev_mem_src. ret = %d\n", ret);
        goto fail;
    }
    bm_image_create(handle, height, width, FORMAT_GRAY, DATA_TYPE_EXT_1N_BYTE, &dst_model_match_model_info, dst_stride);
    ...
    for (i = 0; i < loop_time; i++) {
        ...
        for (u32FrmCnt = 0; u32FrmCnt < u32FrameNumMax; u32FrmCnt++) {
            for (int j = 0; j < height; j++)
                memcpy(srcData + j * stride[0], inputData + (u32FrmCnt * width * height * data_size + j * width * data_size), width * data_size);
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(goldenFg_name, '.') != NULL) {
            int cmp = cmp_u8(goldenFg_name, ive_fg_res, width * height);
            if (cmp != 0) {
                printf("[bmcv ive gmm2] dstFg cmp_u8 failed, cmp = %d \n", cmp);
                //goto fail;
            }
            printf("[bmcv ive gmm2] dstFg cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(ive_fg_res, (unsigned char*)goldenFg_name, width * height);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive gmm2] dstFg cmp_md5 failed, cmp = %d \n", cmp);
                //goto fail;
            }
            printf("[bmcv ive gmm2] dstFg cmp_md5 successful, cmp = %d \n", cmp);
        }
        if (strchr(goldenBg_name, '.') != NULL) {
            int cmp = cmp_u8(goldenBg_name, ive_bg_res, width * height);
            if (cmp != 0) {
                printf("[bmcv ive gmm2] dstBg cmp_u8 failed, cmp = %d \n", cmp);
                //goto fail;
            }
            printf("[bmcv ive gmm2] dstBg cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(ive_bg_res, (unsigned char*)goldenBg_name, width * height);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive gmm2] dstBg cmp_md5 failed, cmp = %d \n", cmp);
                //goto fail;
            }
            printf("[bmcv ive gmm2] dstBg cmp_md5 successful, cmp = %d \n", cmp);
        }
        if (pixel_ctrl) {
            if (strchr(goldenPcMatch_name, '.') != NULL) {
                int cmp = cmp_u8(goldenPcMatch_name, ive_pc_match_res, width * height);
                if (cmp != 0) {
                    printf("[bmcv ive gmm2] pc_match cmp_u8 failed, cmp = %d \n", cmp);
                    //goto fail;
                }
                printf("[bmcv ive gmm2] pc_match cmp_u8 successful, cmp = %d \n", cmp);
            }
            else {
                int cmp = md5_cmp(ive_pc_match_res, (unsigned char*)goldenPcMatch_name, width * height);
                if (cmp != BM_SUCCESS) {
                    printf("[bmcv ive gmm2] pc_match cmp_md5 failed, cmp = %d \n", cmp);
                    //goto fail;
                }
                printf("[bmcv ive gmm2] pc_match cmp_md5 successful, cmp = %d \n", cmp);
            }
        }
        if (bWrite) {
...
int main(int argc, char **argv) {
    ...
    if (argc == 2)
        ...
        %s width height src_fmt pixel_ctrl life_update_enMode src_name inputFactor_name goldenFg_name goldenBg_name goldenPcMatch_name dev_id thread_num loop_num bWrite dstFg_name dstFg_name dstPcMatch_name\n \
