...
static void *ive_erode(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(golden_sample_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive erode] cmp failed, cmp = %d \n", cmp);
                free(iveErodeRes);
                exit(-1);
            }
            printf("[bmcv ive erode] cmp successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(iveErodeRes, (unsigned char*)golden_sample_name, width * height);
            if (cmp != BM_SUCCESS) {
