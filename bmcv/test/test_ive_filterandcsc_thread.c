...
char *dst_name = "filterAndCsc_res.bin";
...
static void *ive_filterandcsc(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(ref_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive filterAndCsc] cmp_u8 failed, cmp = %d \n", cmp);
                bm_image_destroy(&src);
                bm_image_destroy(&dst);
                free(output_ptr);
                exit(-1);
            }
            printf("[bmcv ive filterAndCsc] cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp(output_ptr, (unsigned char*)ref_name, byte_size);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive filterAndCsc] cmp_md5 failed, cmp = %d \n", cmp);
                bm_image_destroy(&src);
                bm_image_destroy(&dst);
                free(output_ptr);
                exit(-1);
...
int main(int argc, char **argv) {
    ...
    if ((argc > 3 && argc < 9) || (argc == 1)) {
        ...
        %s width height csc_type thrSize(0:3x3; 1:5x5) u8Norm src_fmt dst_fmt src_name ref_name dev_id thread_num loop_num bWrite dst_name\n \
