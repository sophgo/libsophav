...
static void *ive_ordStatFilter(void* arg) {
    ...
    if (ctx.i == 0) {
        ...
        if (strchr(golden_sample_name, '.') != NULL) {
            ...
            if (cmp != 0) {
                printf("[bmcv ive ordStatFilter] cmp_u8 failed, cmp = %d \n", cmp);
                free(ordStatFilter_res);
                exit(-1);
            }
            printf("[bmcv ive ordStatFilter] cmp_u8 successful, cmp = %d \n", cmp);
        }
        else {
            int cmp = md5_cmp((unsigned char*)ordStatFilter_res, (unsigned char*)golden_sample_name, width * height);
            if (cmp != BM_SUCCESS) {
                printf("[bmcv ive ordStatFilter] cmp_md5 failed, cmp = %d \n", cmp);
                free(ordStatFilter_res);
                exit(-1);
            }
        }
...
int main(int argc, char **argv) {
    ...
    if (argc == 2)
        ...
        %s 352 288 0 ive_data/00_352x288_y.yuv ive_data/result/sample_OrdStaFilter_Median.yuv 0 1 5 0\n", argv[0], argv[0]);
        exit(-1);
    }
