...
vg_lite_error_t Draw_Image_001()
{
    ...
    vg_lite_buffer_format_t src_formats[] =
    {
        // VG_LITE_BGRA8888,
        // VG_LITE_BGRA4444,
        // VG_LITE_BGRA2222,
    ...
    vg_lite_buffer_format_t dst_formats[] =
    {
        // VG_LITE_BGRA8888,
        VG_LITE_BGRA4444,
        // VG_LITE_BGRA2222,
    ...
    vg_lite_image_mode_t image_modes[] =
    {
    ...
    int32_t src_fmt_cat_count = ARRAY_SIZE(src_formats);
    int32_t dst_fmt_cat_count = ARRAY_SIZE(dst_formats);
    int32_t imagemode_count = ARRAY_SIZE(image_modes);
    int32_t filter_count = ARRAY_SIZE(filter_modes);
    int32_t blend_count = ARRAY_SIZE(blend_modes);

    for (i = 0; i < src_fmt_cat_count; i++) {
        sprintf(name,"Draw_Image_001_%d_",i);
        for (j = 0; j < dst_fmt_cat_count; j++)
            for (k = 0; k < imagemode_count; k++)
...
vg_lite_error_t Draw_Image_002()
{
    ...
    int32_t src_fmts_count = ARRAY_SIZE(src_formats);
    int32_t dst_fmts_count = ARRAY_SIZE(dst_formats);

    for (i = 0; i < src_fmts_count; i++)
        for (j = 0; j < dst_fmts_count; j++)
        {
            src_width = (int32_t)WINDSIZEX;
            src_height = (int32_t)WINDSIZEY;
            dst_width = (int32_t)WINDSIZEX;
