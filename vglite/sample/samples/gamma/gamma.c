/*
 ...
 Alpha Blending: None
 Related APIs: vg_lite_clear/vg_lite_translate/vg_lite_scale/vg_lite_set_gamma/vg_lite_blit
 Description: Load outside landscape image, then blit it to blue dest buffer with different gamma value.
...
static vg_lite_gamma_conversion_t  gamma_value[3] = {  
...
int main(int argc, const char * argv[])
{
    ...
    feature_check = vg_lite_query_feature(gcFEATURE_BIT_VG_GAMMA);
    if (!feature_check) {
        printf("GAMMA is not supported.\n");
        cleanup();
        return -1;
    }
    ...
    fb = &buffer;
    for(i = 0; i < sizeof(gamma_value)/sizeof(gamma_value[0]); i++) {
        ...
        /* Save PNG file. */
        sprintf(fname, "gamma_%d.png", i);
        vg_lite_save_png(fname, fb);
    }

    CHECK_ERROR(vg_lite_set_gamma(gamma_value[0]));
