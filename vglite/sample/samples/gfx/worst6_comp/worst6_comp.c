...
static vg_lite_buffer_t buffer, buffer1, image_buffer, image_buffer2, render_buffer;
...
vg_lite_error_t createBuffers()
{
    ...
    render_buffer.format = VG_LITE_YUY2_TILED;
    render_buffer.width  = 320;
    render_buffer.height = 240;
    render_buffer.stride = 0;
...
int main(int argc, const char * argv[])
{
    ...
    {
        /* Clear the buffer. */
        vg_lite_clear(&render_buffer, NULL, 0xFFFF0000);
        ...
        vg_lite_blit(&render_buffer, &image_buffer2, &matrix2, VG_LITE_BLEND_SRC_OVER, 0, filter);
        render_clock(&render_buffer, 0, 0, 6, 15, 22);
        render_buttons(&render_buffer, 0, 0);

        /* Compose 2 parts to framebuffer */
        vg_lite_blit2(fb, &image_buffer, &render_buffer, &matrix, &matrix2, VG_LITE_BLEND_MULTIPLY, filter);
    }
    ...
    vg_lite_finish();
    vg_lite_save_png("worst6_comp.png", &buffer1);
    /* Cleanup. */
    cleanup();
    return 0;
}
