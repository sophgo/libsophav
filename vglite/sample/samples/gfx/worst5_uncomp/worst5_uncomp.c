...
static vg_lite_buffer_t buffer, image_buffer, image_buffer2, render_buffer, render_buffer2;
...
vg_lite_error_t createBuffers()
{
    ...
    render_buffer.format = render_buffer2.format = VG_LITE_BGRA8888;
    render_buffer.width = render_buffer2.width   = 320;
    render_buffer.height = render_buffer2.height = 240;
...
int main(int argc, const char * argv[])
{
    ...
    for (loops = 0; loops < frames; loops++)
    {
        ...
        vg_lite_blit(&render_buffer, &image_buffer, &matrix, VG_LITE_BLEND_MULTIPLY, 0, filter);
        render_clock(&render_buffer, -80, 0, 0, 15, 8);
        render_buttons(&render_buffer, -80, 0);
        ...
        /* Blit the image using the matrix. */
        vg_lite_blit(&render_buffer2, &image_buffer2, &matrix2, VG_LITE_BLEND_MULTIPLY, 0, filter);
        render_clock(&render_buffer2, 80, 0, 6, 15, 22);
        render_buttons(&render_buffer2, 80, 0);

        /* Compose 2 parts to framebuffer */
        vg_lite_blit2(fb, &render_buffer, &render_buffer2, &matrix, &matrix2, VG_LITE_BLEND_MULTIPLY, filter);
    }
    if (error) {
    ...
    vg_lite_finish();
    /* Save PNG file. */
    vg_lite_save_png("worst_5_uncomp.png", fb);
